...
hostiles = {
    ...
    "missing_witness_rejected": rejected(lambda x: node(x, "exact_evans_history_domain").pop("witness_ref")),
...
checks = {
    ...
    "rigging_cells_constructed": all(node(c, x)["status"] == "constructed" for x in ["exact_evans_history_domain","three_port_block_domain_membership","five_port_arithmetic_summability","maximal_isotropic_evans_domain"]),
    "two_nonrigging_frontier_gates": set(baseline.get("frontier_antichain", [])) == {"prime_shell_adjoint_residual_family","global_fourier_poisson_response_intertwining"},
    "terminal_cut_sets_computed": baseline.get("terminal_cut_sets", {}).get("minimal_blocking_sets") == [["global_fourier_poisson_response_intertwining"],["prime_shell_adjoint_residual_family"]],
