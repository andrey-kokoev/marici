...
and contextual behavior preservation. Four concrete modules now supply finite instances:

- `BoundaryPfaffianFiniteChain.agda` constructs the three-point residual and proves full four- and six-point Pfaffian rank-reset triangles;
- `BoundaryPfaffianResidualFold.agda` constructs arbitrary finite pair extension, alternating charges, odd--odd closure, and metric-level reversal;
- `BoundaryPfaffianResidualRankResetInstance.agda` instantiates the abstract square for arbitrary finite programs generated by pair extension, reversal, and terminal sewing; every program normalizes to one polarized readout, while two explicit probes are jointly faithful;
- `BoundaryPfaffianFluxMomentQuotient.agda` constructs the additive monoidal dihedral action on labelled flux families, the symmetry-stable two-moment kernel, and a split endpoint quotient under an explicit invertible two-seam determinant hypothesis.

The expanded executable interface is frozen in `boundary-pfaffian-constructor-alphabet.v2.json`, with every declared relation bound to an existing evidence artifact.
...
- local Green innovation basis, tridiagonal precision, and determinant transition cocycle;
- distinct Green/Pfaffian line transitions under paired refinement;
- quadratic conditioning bound and the no-go for uniform innovation truncation.
...
- full four- and six-point Pfaffian rank-reset identities;
- arbitrary-size alternating-charge residual fold and odd--odd torsion sewing;
- normalization of every finite ordered/reversal context program to one polarized readout;
- joint faithfulness of the incoming and outgoing probes;
- additive monoidal dihedral labelled-flux quotient and constructive two-seam section.
