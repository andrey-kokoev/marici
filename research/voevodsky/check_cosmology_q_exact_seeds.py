"""Exact-solve all 960 A12 marked-q parity seeds over Q using only T+Q."""
...
RES=ROOT/'research'/'voevodsky'/'results';OUT=RES/'cosmology_q_exact_seeds_a12.json';P=32003
def main():
 ...
 for p in exact.PS:
  base.PRIME=p;sp=list(rees.raw_relations(point,columns));T,_=adapter.derivative_rows(columns,point,td);dx,_=adapter.derivative_rows(columns,point,nx);packets[p]={'T':T,'Q':sp[nI+nK:],'dx':dx}
 ...
 assert len(qdesc)==18720
 seeds=[(qi,kp,lev,e) for qi in range(5) for kp in range(3) for lev in product((1,2),repeat=5) if lev[qi]==1 for e in ((0,0),(0,1),(1,0),(1,1))];assert len(seeds)==960
 records=[];cache={}
 for qi,kp,lev,e in seeds:
  qindex=qdesc.index((qi,kp,lev,e));ti=nI+nK+qindex;rem,tr=dag.trace(packets[P]['dx'][ti],piv);assert not rem;pending={}
  ...
  assert recon==target;records.append({'q_index':qi,'k_pole':kp,'levels':list(lev),'exponent':list(e),'dimension':len(erows),'equations':len(cols),'rank':rank,'max_denominator':max((a.denominator for a in coef),default=1),'full_reconstruction':True})
 base.PRIME=orig
 out={'schema':'marici.voevodsky.cosmology-q-exact-seeds-a12.v1','status':'all_960_q_parity_seeds_verified_over_Q_using_T_plus_Q','ambient_relation_degree':12,'targets_verified':len(records),'dimension_max':max(r['dimension'] for r in records),'equations_max':max(r['equations'] for r in records),'max_denominator':max(r['max_denominator'] for r in records),'records':records,'decision':'Every marked-q pole/level/parity seed has an exact rational T+Q contraction.','limitations':['closures selected by F_32003 pivot order','four-prime CRT reconstruction with exact post-verification','unbounded promotion uses established Q naturality'],'passed':True};OUT.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({k:v for k,v in out.items() if k!='records'},indent=2))
