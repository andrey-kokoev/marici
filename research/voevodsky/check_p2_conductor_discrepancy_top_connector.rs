//! Codimension-three blowup discrepancy gives the required top multiplicity.
//!
...
//!
//! The pairwise-only carrier has defect -2 modulo the primitive K6 top line.
//! For the canonical blowup of the regular conductor center of codimension
//! three, the relative canonical discrepancy is c-1=2.  Adding its primitive
//! top generator with this derived multiplicity closes the cyclic equation.
//! This proves the normalization-provenanced source multiplicity and the
//! resulting saturated carrier closure after the oriented source-top/carrier-
//! top comparison.  That comparison, and the literal entry143 six-functor
//! residue rows, remain outside this certificate.
...
fn main() {
    ...
    let pair_boundaries: Vec<Vec<Int>> = (0..3)
        .map(|column| {
            road_paths.iter().enumerate().fold(
                vec![0; levels[2].len()],
                |mut total, (row, path)| {
    ...
    // The projective conductor source is the exceptional divisor of the
    // blowup of the regular normalization-sheet center J+=(x1,x3,x5).
    // For a regular codimension-c blowup, omega_pi=O((c-1)E); hence c=3
    // independently forces discrepancy 2.  This is geometric multiplicity,
    // not a fitted solution of the carrier equation.
    let conductor_center_codimension = 3_i64;
    let relative_canonical_discrepancy = conductor_center_codimension - 1;
    assert_eq!(relative_canonical_discrepancy, 2);

    // Add one primitive generic-top connector column.  Its column is the
    // primitive associahedral top boundary; the morphism-of-complexes uses
    // it with the forced discrepancy coefficient 2.  Since the pairwise
    // cyclic sum is -2 times that column, the enlarged top equation closes.
    let generic_top_connector = top_generator.clone();
    assert_eq!(generic_top_connector.iter().fold(0, |z, v| gcd(z, *v)), 1);
    let closed_top_equation: Vec<_> = cyclic_filler
        .iter()
        .zip(&generic_top_connector)
        .map(|(pairwise, generic)| pairwise + relative_canonical_discrepancy * generic)
        .collect();
    assert_eq!(closed_top_equation, vec![0; 9]);

    // The enlarged 9x4 column matrix [F0 F1 F2 T] has rank three and a
    // primitive kernel generated by (1,1,1,2).  Its nonzero Smith factors
    // are all units, so adding the geometrically derived top removes both
    // the mod-three obstruction and the former determinant-two defect.
    let enlarged: Matrix = (0..9)
        .map(|row| {
            vec![
                fillers[0][row],
                fillers[1][row],
                fillers[2][row],
                generic_top_connector[row],
            ]
        })
        .collect();
    assert_eq!(rank(&enlarged), 3);
    assert_eq!(minor_gcd(&enlarged, 1), 1);
    assert_eq!(minor_gcd(&enlarged, 2), 1);
    assert_eq!(minor_gcd(&enlarged, 3), 1);
    let kernel_generator = [1_i64, 1, 1, relative_canonical_discrepancy];
    assert_eq!(apply(&enlarged, &kernel_generator), vec![0; enlarged.len()]);
    assert_eq!(
        kernel_generator.into_iter().fold(0, gcd),
        1,
        "the top relation must be primitive"
    );
    ...
    println!(
        "{}",
        r#"{"claim":"The canonical codimension-three conductor blowup forces relative-canonical multiplicity two. After the still-required oriented comparison of its primitive source top with the primitive K6 top line, this multiplicity cancels the pairwise cyclic defect -2 integrally. The enlarged 9x4 carrier matrix has rank three, primitive kernel (1,1,1,2), and nonzero Smith factors [1,1,1].","status":"proved_scoped_discrepancy_multiplicity_and_conditional_carrier_closure","k6_faces":[1,9,21,14],"short_boundary_d2_rank":6,"short_boundary_d2_smith":[1,1,1,1,1,1],"road_difference_augmented_short_rank":7,"road_difference_in_short_image":false,"full_boundary_d2_rank":8,"full_boundary_d2_unit_minor":true,"full_boundary_kernel_rank":1,"p2_pair_incidence":"R-I","c3_orbit_filler_rank":3,"selected_c3_orbit_filler_smith":[1,1,2],"equivariant_filler_smith_family":"[1,1,|-2+3n|]","cyclic_top_coefficient":-2,"equivariant_adjustment_equation":"-2+3n=0","integral_solution":false,"obstruction_mod_3":1,"reflected_endpoint_boundaries_verified":true,"strict_reflection_comparison":"not_reached_after_d3_obstruction","conductor_center_codimension":3,"relative_canonical_discrepancy":2,"generic_top_connector_primitive":true,"enlarged_matrix_rank":3,"enlarged_matrix_smith":[1,1,1],"primitive_kernel":[1,1,1,2],"cyclic_top_closed_after_oriented_top_comparison":true,"oriented_source_top_to_k6_top_comparison":"unconstructed","source_boolean_incidence_count":24,"derived_literal_residue_rows":0,"minimal_additional_geometry":"the generic-top carrier multiplicity is now derived; remaining geometry is the mixed-variance support realization deriving the 24 literal entry143 rows","support_changing_tor_cech_comparison":"unconstructed","literal_mixed_variance_realization":"unconstructed","physical_mapping_fiber":"unconstructed"}"#
    );
}
