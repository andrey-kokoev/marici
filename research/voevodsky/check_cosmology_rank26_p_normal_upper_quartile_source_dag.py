"""Back-substitute the upper-quartile singleton to original S/T rows."""
...
RES=ROOT/'research'/'voevodsky'/'results'; OUT=RES/'cosmology_rank26_p_normal_upper_quartile_source_dag.json'
def main():
 assert rees.AMBIENT==14 and base.PRIME==32003
 samples=json.loads((RES/'cosmology_rank26_p_normal_marked_q_expansion_samples.json').read_text()); target=samples['samples']['upper_quartile']; column=target['column']; assert column==1279 and target['trace_length']==250
 ...
 residue,target_trace=dag.trace({column:1},pivots); assert not residue and len(target_trace)==250
 pending={}
