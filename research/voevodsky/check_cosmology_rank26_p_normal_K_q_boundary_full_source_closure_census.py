...
RES=ROOT/'research'/'voevodsky'/'results';A=rees.AMBIENT;OUT=RES/f'cosmology_rank26_p_normal_K_q_boundary_full_source_closure_census_a{A}.json'
def main():
 assert A in (12,14,16,18) and base.PRIME==32003
 ...
 for kp in (0,1):
  for e in [e for e in base.monomials_at_most(A-4) if sum(e)>=A-6]:
   ...
   records.append({'k_pole':kp,'exponent':list(e),'source_rows':len(source),'T_rows':sum(k[0]=='T' for k in source),'S_K_rows':sum(k[0]=='S_K' for k in source),'Q_rows':sum(k[0]=='Q' for k in source),'retained_columns':len(cols),'maximum_depth':depth,'edges':edges})
 summary={}
 for kp in (0,1):
  rs=[r for r in records if r['k_pole']==kp];m=max(r['source_rows'] for r in rs);summary[f'k{kp}']={'targets':len(rs),'source_rows_min':min(r['source_rows'] for r in rs),'source_rows_max':m,'columns_min':min(r['retained_columns'] for r in rs),'columns_max':max(r['retained_columns'] for r in rs),'q_rows_min':min(r['Q_rows'] for r in rs),'q_rows_max':max(r['Q_rows'] for r in rs),'depth_max':max(r['maximum_depth'] for r in rs),'maximal_records':[r for r in rs if r['source_rows']==m]}
 out={'schema':'marici.voevodsky.cosmology-rank26-p-normal-K-q-boundary-full-source-closure-census.v1','status':'all_boundary_full_source_closures_censused','field':base.PRIME,'ambient_relation_degree':A,'summary':summary,'decision':'Every top-three-degree target has a bounded full-source closure suitable for exact rational solving.','limitations':['p=32003 pivot-selected closures','not exact rational verification'],'passed':True};OUT.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
