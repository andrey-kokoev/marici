"""Back-substitute the maximum-trace singleton to original S/T rows."""
...
RES=ROOT/'research'/'voevodsky'/'results'; OUT=RES/'cosmology_rank26_p_normal_maximum_source_dag.json'
def main():
 assert rees.AMBIENT==14 and base.PRIME==32003
 samples=json.loads((RES/'cosmology_rank26_p_normal_marked_q_expansion_samples.json').read_text()); target=samples['samples']['maximum']; column=target['column']; assert column==16699 and target['trace_length']==544
 ...
 residue,target_trace=dag.trace({column:1},pivots); assert not residue and len(target_trace)==544
 pending={}
