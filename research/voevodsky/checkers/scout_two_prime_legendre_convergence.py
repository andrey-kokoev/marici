...
L=.55;N=80;Q=240;edges=[0,1,2,4,8,16,32,64,128,250,500,1000,2000]
...
for aa,bb in zip(edges[:-1],edges[1:]):
 ...
 if bb in (250,500,1000,2000):
  row={'cutoff':bb}
  for n in (20,40,60,80):row[f'min_eigenvalue_dim_{n}']=float(np.linalg.eigvalsh((H[:n,:n]+endpoint[:n,:n]+(H[:n,:n]+endpoint[:n,:n]).T)/2)[0])
  records.append(row)
...
p=Path(__file__).parents[1]/'results'/'two_prime_legendre_convergence_scout.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
