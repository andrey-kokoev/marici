...
root=Path(__file__).parents[1]/'results';C=np.load(root/'degree8_complete_lower_matrix_L0649_L065.npz')['coefficients'];t=np.cos(np.arange(32,-1,-1)*np.pi/32);vec=[];lam=[];prev=None
for x in t:
...
normpoly[0]-=1;norm_def=float(sum(abs(normpoly)));tail=float(sum(np.linalg.norm(row) for row in P[9:]));gap=9.283012553782547e-9;margin=2.000266669521776e-11;schur=res_bound**2/gap;out={'schema':'marici.voevodsky.moving-frame-polynomial-residual-L0649-L065.v1','frame_degree':32,'post_degree8_frame_coefficient_l1':tail,'normalization_defect_l1_bound':norm_def,'eigen_residual_l1_bound':res_bound,'residual_schur_correction':schur,'critical_margin':margin,'correction_to_margin_ratio':schur/margin,'passed_polynomial_bundle':norm_def<1e-8 and schur<margin/10,'passed':False,'scope':'floating coefficient construction with rigorous-for-stored-coefficients triangle bounds; directed coefficient/source enclosure open','rh_proved':False};np.savez_compressed(root/'moving_frame_polynomial_L0649_L065.npz',frame=P,eigenvalue=lc,residual=R,normalization_defect=normpoly);p=root/'moving_frame_polynomial_residual_L0649_L065.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed_polynomial_bundle']
