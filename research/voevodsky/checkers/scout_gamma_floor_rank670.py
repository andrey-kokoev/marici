...
L=.55;N=670;orders=np.arange(N);norm=2*L*np.sqrt((2*orders+1)/(2*L));ph=(-1.)**(orders//2);Q=300;nodes,wts=leggauss(Q);G=np.zeros((N,N));K=np.zeros((N,N))
...
q250=(digamma(.25+125j).real-math.log(math.pi))/2;A=G+P+E+q250*(np.eye(N)-K);ev=np.linalg.eigvalsh((A+A.T)/2)
out={'schema':'marici.voevodsky.gamma-floor-rank670-scout.v1','cutoff':250,'gamma_floor':q250,'min_eigenvalue':float(ev[0]),'negative_eigenvalues_below_minus_1e-10':int(np.sum(ev<-1e-10)),'smallest_ten':[float(x) for x in ev[:10]],'status':'floating scout; monotonic gamma-floor inequality not interval promoted','passed':True,'rh_proved':False};p=Path(__file__).parents[1]/'results'/'gamma_floor_rank670_scout.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
