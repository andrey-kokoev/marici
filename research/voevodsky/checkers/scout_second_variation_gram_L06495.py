...
VG=At*Bt;factor=16*L/(3*math.pi*(4000-1)**3);tail=factor*VG;alpha=1.067569476012246;S=d['schur'][4];lower=(S-tail/alpha);lower=(lower+lower.T)/2;out={'schema':'marici.voevodsky.second-variation-gram-L06495-scout.v1','physical_nodes':len(x),'weighted_total_variation':At,'variation_gram_norm':float(np.linalg.norm(VG,2)),'second_variation_tail_gram_norm':float(np.linalg.norm(tail,2)),'lower_after_second_variation_tail':float(np.linalg.eigvalsh(lower)[0]),'status':'floating quadrature scout','passed':float(np.linalg.eigvalsh(lower)[0])>0,'rh_proved':False};p=root/'second_variation_gram_L06495_scout.json';p.write_text(json.dumps(out,indent=2)+'\n');np.savez(root/'second_variation_gram_L06495_scout.npz',variation_gram=VG,tail_gram=tail);print(json.dumps(out,indent=2))
