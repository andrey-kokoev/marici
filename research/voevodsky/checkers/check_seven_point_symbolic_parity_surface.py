"""Full tensor parity identity over Q(u,v), off the moment-curve locus."""
...
root=Path(__file__).resolve().parents[1];u,v=s.symbols('u v')
external=[(1,x,x*x,x**3) for x in range(1,7)]+[(1,7,u,v)]
kin=Kinematics(external);dual=prior.parity_kinematics(kin)
...
# The uniform moment curve is used solely for pivot selection.
_,columns=V.subs({u:49,v:343}).rref();assert len(columns)==3
...
report={'passed':True,'field':'Q(u,v)','twistors':[[str(x) for x in row] for row in external],'span_rank':3,'basis_columns':columns,'pivot_rows':rows,'basis_pivot_determinant':str(pivot_det),'full_embedding_entries_checked':35*12,'symmetric_quartic_identities':checks,'signed_weights':list(map(str,weights)),'coordinates':[[str(v) for v in row] for row in coordinates.tolist()],'scope':'Complete Grassmann tensor identity on a two-parameter off-moment-curve family wherever denominators are regular. First six twistors fixed; seventh=(1,7,u,v). Not a generic six-modulus proof. Sampling selects pivots only; all embedding and quartic identities are over Q(u,v).'}
(root/'results/seven-point-symbolic-parity-surface.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({k:report[k] for k in ('passed','field','span_rank','full_embedding_entries_checked','scope')},indent=2))
