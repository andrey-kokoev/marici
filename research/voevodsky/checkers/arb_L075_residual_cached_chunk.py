...
sq=sum((z*z for z in r.values()),arb(0));out={'start':start,'stop':stop,'count':len(inds),'indices':inds,'components':[str(r[m]) for m in inds],'squared_norm':str(sq),'passed':True};p=root/f'L075_residual_cached_chunk_{start}_{stop}.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out))
