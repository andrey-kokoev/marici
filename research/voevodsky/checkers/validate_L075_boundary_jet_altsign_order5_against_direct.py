...
ds=[der(m) for m in range(6+1)];pts=[]
for nn,lam in ((2,math.log(2)),(3,math.log(3)),(4,math.log(2))):
 y=(L-math.log(nn))/L
 for m in range(6+1):D=lam/math.sqrt(nn)/2*ds[m];pts.extend([(m,y,D),(m,-y,(-1)**(m+1)*D)])
def integ(p):
 q={}
...
out={'schema':'marici.voevodsky.L075-boundary-jet-altsign-order5-direct-validation.v1','rows':rows,'passed_scout':all(x['difference_norm']<2e-11 for x in rows),'passed':False,'remaining':'order-six truncation is not a valid approximation at n near 1000; higher jets/cancellation or optimal low-order truncation required','rh_proved':False};p=root/'L075_boundary_jet_altsign-order5_direct_validation.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
