...
p=Path(__file__).parents[1]/'results'/'spherical_bessel_inverse_power_expansions_159.json';p.write_text(json.dumps(payload,indent=2)+'\n');print(json.dumps({'maximum_order':159,'digest':payload['coefficient_digest_sha256'],'max_coefficient_digits':max(len(str(abs(v))) for row in S+C for v in row),'passed':True},indent=2))
