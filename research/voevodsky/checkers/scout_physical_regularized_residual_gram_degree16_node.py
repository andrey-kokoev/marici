...
root=Path(__file__).parents[1]/'results';L=float(sys.argv[1]);tag=sys.argv[2];base=np.load(root/'regularized_union_tail_maps_L0649_L065.npz');P=base['packet'];cc=np.load(root/'degree16_regularized_tail_map_L0649_L065.npz')['coefficients'];D=np.polynomial.chebyshev.chebval((L-.6495)/.0005,cc);Z=P-D;A=np.load(root/f'gamma_floor_{tag}_rank1000_midpoint.npz')['matrix'];S=Z.T@A@Z;N=1000;k=np.arange(N);sc=np.sqrt((2*k+1)/(2*L));qR=(digamma(.25+125j).real-math.log(math.pi))/2;terms=[(math.log(p),math.log(p)/math.sqrt(p)) for p in (2,3)];breaks=sorted(set([-L,L]+[-L+a for a,c in terms]+[L-a for a,c in terms]));xp,wp=roots_legendre(1100);xall=[];wall=[];Hall=[]
...
AZ=H+KZ;G=AZ.T@(wall[:,None]*AZ);PAZ=P.T@A@Z;RG=G-PAZ.T@PAZ;RG=(RG+RG.T)/2;alpha=1.067569476012246;lower=S-RG/alpha;lower=(lower+lower.T)/2;eg=np.linalg.eigvalsh(RG);el=np.linalg.eigvalsh(lower);np.savez(root/f'physical_regularized_residual_gram_{tag}_degree16_refined_matrices.npz',output_gram=G,residual_gram=RG,lower_form=lower)
out={'schema':f'marici.voevodsky.physical-regularized-residual-gram-{tag}-degree16-refined-scout.v1','physical_nodes':len(xall),'frequency_nodes':9*520,'residual_gram_range':[float(eg[0]),float(eg[-1])],'complete_residual_norm':float(math.sqrt(max(0,eg[-1]))),'a_posteriori_lower_form_min':float(el[0]),'status':'floating physical compressed-operator scout','passed':float(eg[0])>-1e-8 and float(el[0])>0,'rh_proved':False};p=root/f'physical_regularized_residual_gram_{tag}_degree16_refined_scout.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed']
