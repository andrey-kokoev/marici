...
flint.ctx.prec=512;root=Path(__file__).parents[1]/'results';d=json.loads((root/'L075_residual_cached_chunk_1000_1100.json').read_text());vv=np.load(root/'normalized_prime_codefect_L075_modes.npz')['vectors'][:150,-1];vv[1::2]=0;v=[arb(repr(float(x))) for x in vv];L=arb('.75')
def peval(y):
...
out={'schema':'marici.voevodsky.L075-continuous-remainder-first-tail-block.v1','degree_range':[1000,1100],'jump_block_norm':str(jsq.sqrt()),'derivative_jump_block_norm':str(dsq.sqrt()),'twice_continuous_remainder_block_norm':str(sq.sqrt()),'full_block_norm':str(arb(d['squared_norm']).sqrt()),'passed_scout':sq.sqrt().upper()<arb('1e-9'),'passed':False,'reason_not_certificate':'one directed block only; infinite continuous remainder needs a second-variation tail bound','rh_proved':False};p=root/'L075_continuous_remainder_first_tail_block.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
