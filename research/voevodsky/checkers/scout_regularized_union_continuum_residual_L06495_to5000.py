...
root=Path(__file__).parents[1]/'results';d=np.load(root/'regularized_union_tail_maps_L0649_L065.npz');U=d['packet']-d['tail_maps'][4];L=.6495;lo=4000;hi=5000;orders=np.arange(hi);norm=2*L*np.sqrt((2*orders+1)/(2*L));ph=(-1.)**(orders//2);Rmat=np.zeros((hi-lo,U.shape[1]));qR=(digamma(.25+125j).real-math.log(math.pi))/2;nodes,wts=leggauss(320)
...
np.savez(root/'regularized_union_continuum_residual_L06495_4000_4999.npz',residual=Rmat);norms=np.linalg.norm(Rmat,axis=0);op=float(np.linalg.svd(Rmat,compute_uv=False)[0]);alpha=1.067569476012246;correction=op*op/alpha
out={'schema':'marici.voevodsky.regularized-union-continuum-residual-L06495-to5000.v1','mode_range':[lo,hi-1],'residual_operator_norm':op,'residual_gram_correction':correction,'target_margin':1.918863145047285e-11,'status':'floating modes 4000..4999 only','passed':correction<1.918863145047285e-11,'rh_proved':False};p=root/'regularized_union_continuum_residual_L06495_to5000.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
