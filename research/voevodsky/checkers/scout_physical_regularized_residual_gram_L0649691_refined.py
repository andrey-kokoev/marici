...
AZ=H+KZ;G=AZ.T@(wall[:,None]*AZ);A=np.load(root/'gamma_floor_L0649691_rank1000_midpoint.npz')['matrix'];PAZ=P.T@A@Z;RG=G-PAZ.T@PAZ;RG=(RG+RG.T)/2;alpha=1.067569476012246;lower=S-RG/alpha;lower=(lower+lower.T)/2;eg=np.linalg.eigvalsh(RG);el=np.linalg.eigvalsh(lower);np.savez(root/'physical_regularized_residual_gram_L0649691_refined_matrices.npz',output_gram=G,residual_gram=RG,lower_form=lower)
out={'schema':'marici.voevodsky.physical-regularized-residual-gram-L0649691-refined-scout.v1','physical_nodes':len(xall),'frequency_nodes':9*520,'residual_gram_range':[float(eg[0]),float(eg[-1])],'complete_residual_norm':float(math.sqrt(max(0,eg[-1]))),'a_posteriori_lower_form_min':float(el[0]),'status':'floating physical compressed-operator scout','passed':float(eg[0])>-1e-8 and float(el[0])>0,'rh_proved':False};p=root/'physical_regularized_residual_gram_L0649691_refined_scout.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed']
