...
np.savez(root/'regularized_union_continuum_residual_L06495_5000_5999.npz',residual=Rmat);norms=np.linalg.norm(Rmat,axis=0);op=float(np.linalg.svd(Rmat,compute_uv=False)[0]);alpha=1.067569476012246;correction=op*op/alpha
out={'schema':'marici.voevodsky.regularized-union-continuum-residual-L06495-to6000.v1','mode_range':[lo,hi-1],'residual_operator_norm':op,'residual_gram_correction':correction,'target_margin':1.918863145047285e-11,'status':'floating modes 5000..5999 only','passed':correction<1.918863145047285e-11,'rh_proved':False};p=root/'regularized_union_continuum_residual_L06495_to6000.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
