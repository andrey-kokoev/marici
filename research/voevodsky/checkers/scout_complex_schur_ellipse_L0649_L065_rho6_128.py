...
out={'maximum_robust_secant_derivative':float(secant),'schema':'marici.voevodsky.complex-schur-ellipse-L0649-L065-rho6-scout128.v1','rho':rho,'samples':rows,'maximum_sampled_schur_abs':max(x['schur_abs'] for x in rows),'minimum_sampled_robust_singular':min(x['robust_min_singular'] for x in rows),'target_schur_abs':1e-7,'passed':max(x['schur_abs'] for x in rows)<1e-7 and min(x['robust_min_singular'] for x in rows)>1e-10,'status':'floating 64-point ellipse scout; boundary enclosure remains required','rh_proved':False};p=root/'complex_schur_ellipse_L0649_L065_rho6_scout128.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
