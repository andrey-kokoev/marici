...
out={'schema':'marici.voevodsky.gate3-E10-source-quadrature-refinement.v1','base_orders':[140,500],'refined_orders':[180,650],'rows':rows,'maximum_change':max(x['maximum_pivot_change'] for x in rows),'maximum_pivot17_change':max(x['pivot17_change'] for x in rows),'target_per_evaluation_error':1e-11,'passed_scout':max(x['maximum_pivot_change'] for x in rows)<1e-11,'passed':False,'rh_proved':False};p=root/'gate3_E10_source_quadrature_refinement.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed_scout']
