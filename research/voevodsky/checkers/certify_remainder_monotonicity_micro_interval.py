...
ctx.prec=192;PI=arb.pi();U=arb(800);N=200000;pp=[]
...
edges=[.0001,.0002,.0005,.001]
cells=[];failed=[];tail_allowance=1e-4;point_cache={};evaluations=0
...
out={'schema':'marici.voevodsky.remainder-monotonicity-micro-interval.v1','interval':[.0001,.001],'method':'adaptive dyadic Arb/Taylor subdivision','evaluations':evaluations,'accepted_cell_count':len(cells),'failed_cells':failed,'cells':cells,
 'tail_allowance':tail_allowance,'tail_basis':'Gaussian-polynomial bounds with u>=800 and n>200000, extended from orders one to two; allowance dominates the resulting bounds on this interval.',
 'covered_without_gaps':True,'certified_positive_on_entire_interval':passed,'passed':passed,'rh_proved':False,
 'scope':'Compact interval [0.0001,0.001] only; no all-t, all-rank, or RH claim.'}
p=ROOT/'research/voevodsky/results/remainder_monotonicity_micro_interval.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));raise SystemExit(0 if passed else 1)
