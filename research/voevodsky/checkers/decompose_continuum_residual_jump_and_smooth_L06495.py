...
ns=np.arange(1000,4000);J=np.zeros((len(ns),40))
for t,row in atoms:
 ker=eval_legendre(ns+1,t)-eval_legendre(ns-1,t);fac=-np.sqrt(L*(2*ns+1)/2)/(2*ns+1);J+=(fac*ker)[:,None]*row
R=np.vstack((np.load(root/'regularized_union_continuum_residual_L06495.npz')['residual'],np.load(root/'regularized_union_continuum_residual_L06495_2000_2999.npz')['residual'],np.load(root/'regularized_union_continuum_residual_L06495_3000_3999.npz')['residual']));Sm=R-J;rows=[]
for q in range(3):
...
out={'schema':'marici.voevodsky.continuum-residual-jump-smooth-decomposition-L06495.v1','rows':rows,'full_minus_jump_frobenius':float(np.linalg.norm(Sm)),'status':'floating exact-formula decomposition','passed':rows[-1]['smooth_remainder_norm']<rows[-1]['full_norm'],'rh_proved':False};p=root/'continuum_residual_jump_smooth_decomposition_L06495.json';p.write_text(json.dumps(out,indent=2)+'\n');np.savez(root/'continuum_residual_jump_smooth_decomposition_L06495.npz',jump=J,smooth=Sm);print(json.dumps(out,indent=2));assert out['passed']
