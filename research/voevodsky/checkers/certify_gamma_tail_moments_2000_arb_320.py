...
try:
 ...
 import flint
 from flint import arb
flint.ctx.prec=512;root=Path(__file__).parents[1]/'results';R=arb(2000);w=arb('1.1');a=2*arb.pi();K=400
...
res={'schema':'marici.voevodsky.gamma-tail-moments-250-arb.v1','precision_bits':flint.ctx.prec,'R':2000,'terminal_index':K,'power_range':[2,320],'moments':out,'maximum_moment_radius':maxrad,'passed':True,'rh_proved':False}
p=root/'gamma_tail_moments_2000_arb_320.json';p.write_text(json.dumps(res,indent=2)+'\n');print(json.dumps({k:res[k] for k in ('precision_bits','terminal_index','power_range','maximum_moment_radius','passed')},indent=2));assert res['passed']
