...
for theta in np.linspace(0,2*np.pi,65)[:-1]:
 z=.5*(rho*np.exp(1j*theta)+rho**-1*np.exp(-1j*theta));L=center+radius*z;M=block(L);F=M[:19,:19];b=M[:19,19];s=M[19,19]-M[19,:19]@np.linalg.solve(F,b);rows.append({'theta':float(theta),'L_real':float(L.real),'L_imag':float(L.imag),'schur_real':float(s.real),'schur_imag':float(s.imag),'schur_abs':float(abs(s)),'robust_min_singular':float(svdvals(F)[-1])})
out={'schema':'marici.voevodsky.complex-schur-ellipse-L0649-L065-scout64.v1','rho':rho,'samples':rows,'maximum_sampled_schur_abs':max(x['schur_abs'] for x in rows),'minimum_sampled_robust_singular':min(x['robust_min_singular'] for x in rows),'target_schur_abs':1e-7,'passed':max(x['schur_abs'] for x in rows)<1e-7 and min(x['robust_min_singular'] for x in rows)>1e-10,'status':'floating 64-point ellipse scout; boundary enclosure remains required','rh_proved':False};p=root/'complex_schur_ellipse_L0649_L065_scout64.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
