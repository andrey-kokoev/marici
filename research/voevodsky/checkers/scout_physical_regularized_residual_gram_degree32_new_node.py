...
root=Path(__file__).parents[1]/'results';L=float(sys.argv[1]);tag=sys.argv[2];base=np.load(root/'regularized_union_tail_maps_L0649_L065.npz');P=base['packet'];new=np.load(root/'degree32_new_tail_maps_L0649_L065.npz');idx=int(sys.argv[3]);D=new['tail_maps'][idx];Z=P-D;A=np.load(root/f'gamma_floor_{tag}_rank1000_midpoint.npz')['matrix'];S=Z.T@A@Z;N=1000;k=np.arange(N);sc=np.sqrt((2*k+1)/(2*L));qR=(digamma(.25+125j).real-math.log(math.pi))/2;terms=[(math.log(p),math.log(p)/math.sqrt(p)) for p in (2,3)];breaks=sorted(set([-L,L]+[-L+a for a,c in terms]+[L-a for a,c in terms]));xp,wp=roots_legendre(1100);xall=[];wall=[];Hall=[]
