...
root=Path(__file__).parents[1]/'results';Aend=np.load(root/'gamma_floor_L065_rank1000_midpoint.npz')['matrix'];ev,V=np.linalg.eigh(Aend);W=np.column_stack((V[:,1:20],V[:,0]));N=1000;o=np.arange(N);zg,wg=leggauss(140);xs,ws=leggauss(500);rho=5.;center=.6495;radius=.0005;q250=(digamma(.25+125j).real-math.log(math.pi))/2
...
out={'maximum_robust_secant_derivative':float(secant),'schema':'marici.voevodsky.complex-schur-ellipse-L0649-L065-rho5-scout64.v1','rho':rho,'samples':rows,'maximum_sampled_schur_abs':max(x['schur_abs'] for x in rows),'minimum_sampled_robust_singular':min(x['robust_min_singular'] for x in rows),'target_schur_abs':1e-7,'passed':max(x['schur_abs'] for x in rows)<1e-7 and min(x['robust_min_singular'] for x in rows)>1e-10,'status':'floating 64-point ellipse scout; boundary enclosure remains required','rh_proved':False};p=root/'complex_schur_ellipse_L0649_L065_rho5_scout64.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
