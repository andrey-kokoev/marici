...
out={'schema':'marici.voevodsky.remainder-monotonicity-medium-interval.v1','interval':[.25,1.0],'method':'adaptive dyadic Arb/Taylor subdivision','evaluations':evaluations,'accepted_cell_count':len(cells),'failed_cells':failed,'cells':cells,
 ...
 'scope':'Compact interval [0.25,1.0] only; no all-t, all-rank, or RH claim.'}
p=ROOT/'research/voevodsky/results/remainder_monotonicity_medium_interval.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));raise SystemExit(0 if passed else 1)
