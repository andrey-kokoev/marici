...
L=.55;N=160;Q=240;edges=[0,1,2,4,8,16,32,64,128,250,500,1000,2000]
...
for aa,bb in zip(edges[:-1],edges[1:]):
 ...
 if bb in (250,500,1000,2000):
  row={'cutoff':bb}
  for n in (80,120,160):row[f'min_eigenvalue_dim_{n}']=float(np.linalg.eigvalsh((H[:n,:n]+endpoint[:n,:n]+(H[:n,:n]+endpoint[:n,:n]).T)/2)[0])
  records.append(row)
Full=(H+endpoint);Full=(Full+Full.T)/2;A=Full[:80,:80];B=Full[:80,80:];D=Full[80:,80:];Schur=D-B.T@np.linalg.solve(A,B);se=np.linalg.eigvalsh((Schur+Schur.T)/2)
out={'schema':'marici.voevodsky.two-prime-legendre-convergence-scout.v1','L':L,'gauss_nodes_per_panel':Q,'records':records,'rank160_over_rank80_schur':{'minimum_eigenvalue':float(se[0]),'maximum_eigenvalue':float(se[-1])},'status':'floating stability scout; no directed tail enclosure','passed':True,'rh_proved':False}
p=Path(__file__).parents[1]/'results'/'two_prime_legendre_convergence_160_scout.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
