...
root=Path(__file__).parents[1]/'results';d=json.loads((root/'complex_schur_ellipse_L0649_L065_rho5_scout64.json').read_text());r=d['samples'];z=np.array([complex(x['L_real'],x['L_imag']) for x in r]);half=max(abs(z[(i+1)%len(z)]-z[i]) for i in range(len(z)))/2
