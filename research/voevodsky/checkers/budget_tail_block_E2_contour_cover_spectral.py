...
t=np.cos(np.arange(32,-1,-1)*np.pi/32);C=np.polynomial.chebyshev.chebfit(t,np.array(X).reshape(33,-1),16).reshape(17,960,960);rho=2.;den=rho-rho**-1;d_dz=sum(np.linalg.norm(C[k],2)*k*(rho**k-rho**(-k))/den for k in range(1,17));d_dtheta=d_dz*(rho+rho**-1)/2;sample_floor=1.0673377253416951;N=math.ceil(math.pi*d_dtheta/(sample_floor*.5));half_arc=math.pi/N;variation=d_dtheta*half_arc;out={'schema':'marici.voevodsky.tail-block-E2-contour-cover-budget.v1','spectral_derivative_bound_dC_dz':float(d_dz),'spectral_derivative_bound_dC_dtheta':float(d_dtheta),'sampled_floor_reference':sample_floor,'recommended_samples':N,'half_arc_variation_bound':float(variation),'conditional_singular_floor':float(sample_floor-variation),'passed_conditionally':bool(variation<sample_floor),'passed':False,'conditions':['directed coefficient spectral-norm upper bounds','directed singular lower bounds at recommended contour nodes','post-degree16 source tail included'],'rh_proved':False};np.savez_compressed(root/'tail_block_degree16_polynomial_L0649_L065.npz',coefficients=C);p=root/'tail_block_E2_contour_cover_budget.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed_conditionally']
