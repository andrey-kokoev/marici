...
try:
 ...
 from flint import arb
root=Path(__file__).parents[1]/'results';C=np.load(root/'corrected_degree16_complete_lower_L0649_L065.npz')['coefficients'];tn=np.cos(np.arange(32,-1,-1)*np.pi/32);vals=[]
for t in tn:
...
out={'schema':'marici.voevodsky.corrected-robust-branch-polynomial-arb-L0649-L065.v1','degree':32,'coefficients':[float(x) for x in coef],'post_degree8_l1':float(sum(abs(coef[9:]))),'interval_count':N,'minimum_interval_lower':min(lows),'passed_polynomial':min(lows)>0,'source_matrix_enclosed':False,'passed':False,'scope':'directed positivity of stored robust eigenbranch polynomial; matrix interpolation/source remainder not enclosed','rh_proved':False};p=root/'corrected_robust_branch_polynomial_arb_L0649_L065.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({k:v for k,v in out.items() if k!='coefficients'},indent=2))
