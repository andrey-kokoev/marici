...
root=Path(__file__).parents[1]/'results';d=np.load(root/'regularized_union_tail_maps_L0649_L065.npz');P=d['packet'];Z=P-d['tail_maps'][4];S=d['schur'][4];L=.6495;N=1000;k=np.arange(N);sc=np.sqrt((2*k+1)/(2*L));qR=(digamma(.25+125j).real-math.log(math.pi))/2;terms=[(math.log(p),math.log(p)/math.sqrt(p)) for p in (2,3)];breaks=sorted(set([-L,L]+[-L+a for a,c in terms]+[L-a for a,c in terms]));xp,wp=roots_legendre(1300);xall=[];wall=[];Hall=[]
...
xall=np.concatenate(xall);wall=np.concatenate(wall);H=np.vstack(Hall);KZ=np.zeros_like(H);o=np.arange(N);norm=2*L*np.sqrt((2*o+1)/(2*L));phase=np.array([(-1)**(n//2)*(1 if n%2==0 else -1j) for n in o]);z,w=roots_legendre(620)
...
AZ=H+KZ;G=AZ.T@(wall[:,None]*AZ);A=np.load(root/'gamma_floor_L06495_rank1000_midpoint.npz')['matrix'];PAZ=P.T@A@Z;RG=G-PAZ.T@PAZ;RG=(RG+RG.T)/2;alpha=1.067569476012246;lower=S-RG/alpha;lower=(lower+lower.T)/2;eg=np.linalg.eigvalsh(RG);el=np.linalg.eigvalsh(lower);np.savez(root/'physical_regularized_residual_gram_L06495_refined2_matrices.npz',output_gram=G,residual_gram=RG,lower_form=lower)
out={'schema':'marici.voevodsky.physical-regularized-residual-gram-L06495-refined2-scout.v1','physical_nodes':len(xall),'frequency_nodes':9*620,'residual_gram_range':[float(eg[0]),float(eg[-1])],'complete_residual_norm':float(math.sqrt(max(0,eg[-1]))),'a_posteriori_lower_form_min':float(el[0]),'status':'floating physical compressed-operator scout','passed':float(eg[0])>-1e-8 and float(el[0])>0,'rh_proved':False};p=root/'physical_regularized_residual_gram_L06495_refined2_scout.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed']
