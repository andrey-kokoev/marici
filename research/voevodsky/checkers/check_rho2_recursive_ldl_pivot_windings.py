...
root=Path(__file__).parents[1]/'results';d=json.loads((root/'complex_schur_ellipse_L0649_L065_rho2_scout64.json').read_text());r=d['samples'];rows=[]
for k in range(18):
...
out={'schema':'marici.voevodsky.rho2-recursive-ldl-pivot-windings.v1','rows':rows,'all_zero_winding':all(abs(x['winding'])<1e-12 for x in rows),'all_phase_increments_below_pi':all(x['maximum_phase_increment']<np.pi for x in rows),'passed':all(abs(x['winding'])<1e-12 and x['maximum_phase_increment']<np.pi for x in rows),'rh_proved':False};p=root/'rho2_recursive_ldl_pivot_windings.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
