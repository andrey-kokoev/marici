#!/usr/bin/env python3
"""Moving second eigenline for corrected degree-16 slab polynomial."""
import json,sys
...
for x in t:
 H=np.polynomial.chebyshev.chebval(x,C);e,V=np.linalg.eigh((H+H.T)/2);v=V[:,1]
 if prev is not None and v@prev<0:v=-v
 Vv.append(v);ee.append(e[1]);prev=v
...
normpoly[0]-=1;res=sum(np.linalg.norm(x) for x in R);nd=sum(abs(normpoly));grid=np.linspace(-1,1,4001);mn=min(np.polynomial.chebyshev.chebval(grid,lc));out={'schema':'marici.voevodsky.second-moving-line-corrected-L0649-L065.v1','degree':32,'normalization_defect_bound':float(nd),'eigen_residual_bound':float(res),'minimum_branch':float(mn),'post_degree16_frame_l1':float(sum(np.linalg.norm(x) for x in P[17:])),'passed_scout':bool(nd<1e-10 and res<1e-10 and mn>0),'passed':False,'rh_proved':False};np.savez_compressed(root/'second_moving_line_corrected_L0649_L065.npz',frame=P,eigenvalue=lc,residual=R,normalization_defect=normpoly);p=root/'second_moving_line_corrected_L0649_L065.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed_scout']
