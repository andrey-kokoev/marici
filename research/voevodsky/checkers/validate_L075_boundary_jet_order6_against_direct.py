...
out={'schema':'marici.voevodsky.L075-boundary-jet-order6-direct-validation.v1','rows':rows,'passed_scout':all(x['difference_norm']<2e-11 for x in rows),'passed':False,'remaining':'order-six truncation is not a valid approximation at n near 1000; higher jets/cancellation or optimal low-order truncation required','rh_proved':False};p=root/'L075_boundary_jet_order6_direct_validation.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
