...
norms=np.linalg.norm(Rmat,axis=0);A1000=np.load(root/'gamma_floor_rank1000_midpoint.npz')['matrix'];R1=A1000[670:,:670]@U;R=np.vstack((R1,Rmat));np.savez(root/'two_prime_regularized_residual_through_1999.npz',residual=R);alpha=.5897761979384148;S=d['finite_schur'];lower=(S-R.T@R/alpha);lower=(lower+lower.T)/2;ev=np.linalg.eigvalsh(lower)
out={'schema':'marici.voevodsky.two-prime-regularized-residual-to-2000-scout.v1','new_mode_range':[lo,hi-1],'new_block_norm':float(np.linalg.svd(Rmat,compute_uv=False)[0]),'combined_residual_norm':float(np.linalg.svd(R,compute_uv=False)[0]),'a_posteriori_lower_form_min':float(ev[0]),'status':'floating; modes >=2000 omitted','passed':float(ev[0])>0,'rh_proved':False};p=root/'two_prime_regularized_residual_to_2000_scout.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed']
