...
try:
 ...
 import flint
 from flint import arb
flint.ctx.prec=1600;start=int(sys.argv[1]);stop=int(sys.argv[2]);root=Path(__file__).parents[1]/'results';mom=json.loads((root/'gamma_tail_moments_250_arb_320.json').read_text())['moments'];bc=json.loads((root/'spherical_bessel_inverse_power_expansions_159.json').read_text());S=bc['S'];C=bc['C'];L=arb('.55');binet=arb('6.60e-39') # doubled rank amplitude maximum
...
for i in range(start,stop):
 ...
 rows[str(i)]=row
out={'schema':'marici.voevodsky.gamma-tail-250-arb-160-chunk.v1','start':start,'stop_exclusive':stop,'rows':rows,'maximum_entry_radius':maxrad,'budget':'5e-11','passed':maxrad<5e-11}
p=root/f'gamma_tail_250_arb_160_rows_{start}_{stop-1}.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'start':start,'stop':stop,'maximum_entry_radius':maxrad,'passed':out['passed']},indent=2))
