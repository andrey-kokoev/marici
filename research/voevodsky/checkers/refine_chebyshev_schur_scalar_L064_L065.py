...
t=np.cos(np.arange(8,-1,-1)*np.pi/8);coef=np.polynomial.chebyshev.chebfit(t,np.array(vals),8);grid=np.linspace(-1,1,2001);sg=np.polynomial.chebyshev.chebval(grid,coef);out={'schema':'marici.voevodsky.chebyshev-schur-scalar-L064-L065.v1','node_schur_values':vals,'node_robust_block_minima':rob,'chebyshev_coefficients':[float(x) for x in coef],'high_degree_absolute_sum_5_to_8':float(np.sum(abs(coef[5:]))),'dense_schur_min':float(np.min(sg)),'dense_min_L':float(.645+.005*grid[int(np.argmin(sg))]),'status':'floating scalar Schur continuation; directed analytic remainder open','passed':float(np.min(sg))>0 and min(rob)>0,'rh_proved':False};p=root/'chebyshev_schur_scalar_L064_L065.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed']
