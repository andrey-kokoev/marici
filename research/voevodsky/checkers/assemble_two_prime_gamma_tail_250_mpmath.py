...
mp.mp.dps=50;R=mp.mpf(250);L=mp.mpf('0.55');omega=2*L
...
out={'schema':'marici.voevodsky.two-prime-gamma-tail-mpmath.v1','precision_decimal_digits':mp.mp.dps,'tail_anchor':'250','dimension':80,'smallest_tail_eigenvalue':mp.nstr(ev[0],30),'largest_tail_eigenvalue':mp.nstr(ev[79],30),'maximum_absolute_entry':mp.nstr(max(abs(M[i,j]) for i in range(80) for j in range(80)),30),'status':'high-precision asymptotic assembly; Binet remainder budget external; no ball rounding','passed':True,'rh_proved':False}
p=root/'two_prime_gamma_tail_250_mpmath.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
