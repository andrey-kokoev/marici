...
try:
 ...
 import flint
 from flint import arb,arb_mat
flint.ctx.prec=1024;N=160;Q=160;L=arb('0.55')
def legs(x):
 out=[arb(1),x]
...
out={'schema':'marici.voevodsky.two-prime-overlap-arb.v1','precision_bits':flint.ctx.prec,'dimension':N,'gauss_order':Q,'polynomial_degree_exactness':319,'active_primes':[2,3],'maximum_entry_radius':maxrad,'maximum_entry_absolute_upper':maxabs,'budget':'2e-11','budget_met':maxrad<2e-11,'matrix':[[str(P[i,j]) for j in range(N)] for i in range(N)],'passed':maxrad<2e-11,'rh_proved':False}
p=Path(__file__).parents[1]/'results'/'two_prime_overlap_arb_160.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));assert out['passed']
