...
module ClosureAppendInduction where
...
module Induction (K : Type) (Piece : K → Type) (Boundary : K → K → Type)
  ...
  module C = Coherence K Piece Boundary attachL attachR
  open C.D.G.G.A.N
  ...
  module Step (a : K) {b c d e f g : K}
    ...
    Pointed : Type
    Pointed = Σ[ F ∈ (P.Start → Normal (cons a ((u ++ v) ++ w))) ]
      ((x : Piece a) → F (U.sourceFirst x) ≡ PO.inl x)

    leftStart leftEnd rightStart rightEnd : Pointed
    leftStart = U.W.leftFlat , λ x → refl
    leftEnd = (λ x → equivFun LL.equivalence (pre x)) , λ x → refl
    rightStart = (λ x → U.W.returnIndex (U.W.rightFlat (U.W.Raw.associate x))) , U.rightFirst
    rightEnd = (λ x → equivFun RR.equivalence (pre x)) , λ x → refl
    ...
    leftWhisker (PO.inl x) = refl
    leftWhisker (PO.inr x) = refl
    leftWhisker (PO.push z i) j = sym (rUnit (PO.push z)) j i
    ...
    leftFactor =
      (λ i → (λ x → equivFun (appendFrame ((cons a u) ++ v) w)
        (equivFun (leftChildren (~ i)) x)) , (λ x → refl))
      ∙ (λ i → (λ x → equivFun (appendFrame ((cons a u) ++ v) w)
        (LC.compositionAt x (~ i))) , (λ x → refl))
      ∙ (λ i → (λ x → equivFun LCompose.Second.equivalence
        (LN.naturalityAt (equivFun LC.First.equivalence x) i)) , (λ x → refl))
      ∙ (λ i → (λ x → LCompose.compositionAt
        (P.A-BC-D.associate (equivFun LC.First.equivalence x)) i) , (λ x → refl))
      ∙ (λ i → (λ x → equivFun LL.equivalence
        (P.A-BC-D.associate (leftWhisker x i))) , (λ x → refl))
    ...
    rightWhisker (PO.inl x) = refl
    rightWhisker (PO.inr x) = refl
    rightWhisker (PO.push s i) j = sym (rUnit (PO.push s)) j i
    ...
    rightFactor =
      (λ i → (λ x → U.W.returnIndex (U.W.rightFlat (U.W.Raw.associate x))) ,
        (λ x → sym (lUnit (Index.firstWitness x)) i))
      ∙ (λ i → (λ x → equivFun (Index.framePath i) (U.W.rightFlat (U.W.Raw.associate x))) ,
        (λ x → Index.firstWitnessPath x i))
      ∙ (λ i → (λ x → equivFun Index.comparison (equivFun RCompose.Second.equivalence
        (RN.naturalityAt (P.AB-C-D.associate x) i))) , (λ x → refl))
      ∙ (λ i → (λ x → equivFun Index.comparison
        (RCompose.compositionAt (P.shortRoute x) i)) , (λ x → refl))
      ∙ (λ i → (λ x → equivFun Index.comparison
        (equivFun (rightChildren i) (P.shortRoute x))) , (λ x → refl))
      ∙ (λ i → (λ x → Outside.compositionAt (P.shortRoute x) i) , (λ x → refl))
      ∙ (λ i → (λ x → equivFun Outside.Combined.equivalence (P.pentagonAt x (~ i))) , (λ x → refl))
      ∙ (λ i → (λ x → equivFun Outside.Combined.equivalence (rightWhisker (pre x) (~ i))) , (λ x → refl))
      ∙ (λ i → (λ x → LastCompose.compositionAt (pre x) i) , (λ x → refl))
      ∙ (λ i → (λ x → equivFun (lastChildren i) (pre x)) , (λ x → refl))

    step : L.AppendData → U.AppendData
    step ih = U.pointed (λ x i → fst (whole i) x) (λ x i → snd (whole i) x)
      where
      ...
      whole : rightStart ≡ (U.W.leftFlat , U.leftFirst)
      whole = rightFactor
        ∙ (λ i → (λ x → equivFun (lifted i) (pre x)) , (λ x → refl))
        ∙ sym leftFactor
        ∙ (λ i → U.W.leftFlat , (λ x → rUnit refl i))
