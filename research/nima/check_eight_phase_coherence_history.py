#!/usr/bin/env python3
"""Exact connected eight-phase coherence-history toy model."""
...
k=8;n=3*k+2;xs=[s.Integer(i*i+3*i+1) for i in range(1,n+1)];Z={i:s.Matrix([1,x,x*x,x**3]) for i,x in enumerate(xs,1)};S={i:external_supertwistor(i,Z[i]) for i in Z}
...
checks={'eight_degree_four_phases':len(phases)==k,'history_degree_thirty_two':4*k==32,'connected_overlap_chain':overlaps==expected,'generic_component_nonzero':value!=0,'antisymmetric_component_orientation':reversed_value==-value,'determinant_evaluation_avoids_enumeration':elapsed<5}
out={'schema':'marici.nima.eight-phase-coherence-history.v1','phase_labels':[list(q) for q in labels],'phase_degrees':[4]*k,'history_degree':4*k,'shared_coherence_labels':overlaps,'component_labels':[list(x) for x in component_labels],'component_value':str(value),'implicit_assignment_terms':math.factorial(k)**4,'evaluation_seconds':elapsed,'evaluation':'product of four 8x8 determinants','checks':checks,'passed':all(checks.values()),'scope':'Exact selected degree-thirty-two component of a connected eight-phase history; compositional toy invariant, not a sourced full N8MHV amplitude.'}
p=ROOT/'research/nima/results/eight-phase-coherence-history.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2));raise SystemExit(0 if out['passed'] else 1)
