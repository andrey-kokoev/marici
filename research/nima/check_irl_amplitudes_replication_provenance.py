...
checks={'manifest_passed':manifest['passed'],'twelve_families':manifest['family_count']==12,'two_files_per_family':len(files)==24,'all_nonempty':all(r['bytes']>0 for r in files),'all_digests_unique':len({r['sha256'] for r in files})==len(files)}
