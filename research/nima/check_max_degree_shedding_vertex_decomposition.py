...
exec(compile(open(__file__.replace('check_max_degree_shedding_vertex_decomposition.py','check_common_tree_vertex_decomposability.py'),encoding='utf-8').read().split('rows=[]')[0], 'vd-prefix', 'exec'))
...
def dec(fs):
 ...
 V,N=graph(fs);mx=max(map(lambda v:len(N[v]),V));vs=sorted((v for v in V if len(N[v])==mx),key=lambda q:(len(q),tuple(sorted(q))));c=[]
 for v in vs:
  ok,de,li=data(fs,v)
  if ok:c.append((v,de,li))
 if not c:terminal.append({'facets':len(fs),'vertices':len(V),'max_degree':mx,'degree_sequence':sorted(len(N[v]) for v in V)});cache[k]=False;return False
 v,de,li=c[0];cache[k]=dec(de) and dec(li);return cache[k]
rows=[];fails=[]
for n in range(4,9):
...
out={'schema':'marici.nima.max-degree-shedding-vertex-decomposition.v1','range':[4,8],'results':rows,'states':len(cache),'terminal_obstructions':terminal[:20],'failures':fails[:20],'passed':not fails}
p=ROOT/'research/nima/results/max-degree-shedding-vertex-decomposition.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'passed':out['passed'],'results':rows,'states':len(cache),'first_terminal':terminal[:1]},indent=2));raise SystemExit(0 if out['passed'] else 1)
