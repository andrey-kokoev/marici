#!/usr/bin/env python3
"""Exact projectivity of the seven-point planar scattering 4-form."""
import json,sys
...
import sympy as s
N=7;coords=s.symbols('x3:7');C={(i,j):s.Symbol(f'c{i}{j}') for i in range(1,N-1) for j in range(i+2,N)}
...
checks={'fortytwo_cubic_graph_terms':len(terms)==42,'fourteen_channels':len([d for d,v in X.items() if v!=0])==14,'unit_graph_orientations':all(abs(o)==1 for o,_ in terms),'nonzero_scattering_form':len(original)==42,'local_rescaling_variation_zero':not variation}
report={'schema':'marici.nima.abhy-seven-point-scattering-form-projectivity.v1','benchmark':{'paper':'Arkani-Hamed, Bai, He, Yan, Scattering Forms and the Positive Geometry of Kinematics, Color and the Worldsheet','arxiv':'1711.09102','result':'projectivity of the seven-point planar scattering form'},'term_count':len(terms),'oriented_terms':[{'sign':o,'channels':list(q)} for o,q in terms],'transformation':'dlog X_a -> dlog X_a + dlog Lambda','variation_coefficients':{str(k):v for k,v in variation.items()},'checks':checks,'passed':all(checks.values()),'scope':'Exact exterior-algebra projectivity check for the seven-point planar scattering 4-form.'}
out=ROOT/'research/nima/results/abhy-seven-point-scattering-form-projectivity.json';out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'passed':report['passed'],'checks':checks,'variation_terms':len(variation)},indent=2));raise SystemExit(0 if report['passed'] else 1)
