...
$Receipt = Join-Path $Results 'wolfram-converse-formal-audit.json'
if (Test-Path $Receipt) { Remove-Item $Receipt }
Push-Location $Root
try {
  & (Join-Path $PSScriptRoot 'check_cubical_agda.ps1') -Module QBooleanNandBridge -Fresh
  if ($LASTEXITCODE -ne 0) { throw 'Positive compilation failed' }
  $PositivePath = Join-Path $Results 'agda-QBooleanNandBridge.json'
  ...
  $Controls = @()
  foreach ($Name in @('WolframBadConjunction', 'WolframBadProof')) {
    ...
    $Expected = if ($Name -eq 'WolframBadConjunction') { 'false != true' } else { '[xy] != [xy]' }
    if ($Code -eq 0 -or !$Text.Contains('[UnequalTerms]') -or $Text -notmatch $Expected -or !$Text.Contains('refl')) {
  ...
  @{ status='boolean-nand-equational-equivalence-checked'; finished_at=[DateTime]::UtcNow.ToString('o');
     positive_receipt_sha256=(Get-FileHash -Algorithm SHA256 $PositivePath).Hash;
     controls=$Controls;
     scope='Nonempty set with Wolfram operation gives complemented bounded distributive lattice; original operation recovered as NAND. Boolean lattice NAND satisfies W. All five reconstructed operations/constants roundtrip. Q truth-type bridge checked.'
