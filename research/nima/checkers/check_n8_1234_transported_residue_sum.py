#!/usr/bin/env python3
"""Exact local transported-residue sum over the six-facet <Y1234> source facets."""
from pathlib import Path
import json,sys
R=Path(__file__).resolve().parents[3];sys.path.insert(0,str(R/'research/benincasa/.tmp_sympy'));import sympy as s
ori=json.loads((R/'research/nima/results/n8-physical-residue-orientations.json').read_text());fib=json.loads((R/'research/nima/results/n8-physical-facet-inverse-fibers.json').read_text());matches=json.loads((R/'research/nima/results/eight-point-history-positroid-matching.json').read_text())['matches'];byh={x['history_index']:x for x in matches};rep=next(x for x in ori['representatives'] if x['physical_bracket']==[1,2,3,4]);fo=next(x for x in fib['orbits'] if x['physical_bracket']==[1,2,3,4]);u=s.symbols('u1:9');n=8
...
for f in rep['source_facets']:
 start,coord,osign=f['exposing_chart']['cyclic_start'],f['exposing_chart']['coordinate'],f['seed_oriented_residue_sign']
 if f['history_index']==2:start,coord,osign=5,1,1
 C=chart(byh[f['history_index']]['cell_key'],start,u,coord-1);Y=C*Z;N=Y[:,list(piv)].inv()*Y;tc=[N[i,j] for i,j in target_rows];free=[z for i,z in enumerate(u) if i!=coord-1];# recover source coordinates from the pre-solved affine fiber branch
 ff=next(z for z in fo['facets'] if z['history_index']==f['history_index']);J=s.Matrix(tc).jacobian(free)*s.diag(*free)
 for branch,q0 in enumerate(z for z in ff['fiber_solutions'] if z['exact_open_facet_matroid']):
  q=q0['variables'];xf={z:s.Rational(q[str(z)]) for z in x};Cf=Ysample*L+X.subs(xf)*K;alluv=extract(byh[f['history_index']]['cell_key'],start,Cf);uv={z:alluv[i] for i,z in enumerate(u) if i!=coord-1};Jn=J.subs(uv)
  if chosen is None:
   chosen=[]
   for qi in range(8):
    if s.Matrix([Jn[i,:] for i in chosen+[qi]]).rank()>len(chosen):chosen.append(qi)
    if len(chosen)==7:break
  det=s.factor(s.Matrix([Jn[i,:] for i in chosen]).det());coef=s.factor(s.Integer(osign)/det);entries.append({'history_index':f['history_index'],'branch':branch,'positive_branch':q0['positive'],'source_parameters':{str(z):str(s.factor(uv[z])) for z in free},'target_log_jacobian':str(det),'orientation_sign':osign,'transported_coefficient':str(coef)});print(f['history_index'],branch,det,coef,flush=True)
total=s.factor(sum(s.sympify(x['transported_coefficient']) for x in entries));checks={'six_open_facet_branches':len(entries)==6,'common_target_chart_rank_seven':len(chosen)==7,'nonzero_jacobians':all(x['target_log_jacobian']!='0' for x in entries),'nonzero_total':total!=0};out={'schema':'marici.nima.n8-1234-transported-residue-sum.v1','physical_bracket':[1,2,3,4],'target_pivot':[x+1 for x in piv],'target_coordinate_indices':chosen,'target_sample':fo['target_sample'],'contributions':entries,'summed_coefficient':str(total),'checks':checks,'passed':all(checks.values()),'claim_boundary':'This is an exact pointwise evaluation in seven ordinary normalized target coordinates. It is not yet a symbolic identity or comparison with an independent closed-form boundary canonical form.'};p=R/'research/nima/results/n8-1234-transported-residue-sum.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'summed_coefficient':str(total),'checks':checks,'passed':out['passed']},indent=2));raise SystemExit(0 if out['passed'] else 1)
