#!/usr/bin/env python3
"""Exact local transported-residue sum over the two <Y1256> source facets."""
from pathlib import Path
import json,sys
R=Path(__file__).resolve().parents[3];sys.path.insert(0,str(R/'research/benincasa/.tmp_sympy'));import sympy as s
ori=json.loads((R/'research/nima/results/n8-physical-residue-orientations.json').read_text());fib=json.loads((R/'research/nima/results/n8-physical-facet-inverse-fibers.json').read_text());matches=json.loads((R/'research/nima/results/eight-point-history-positroid-matching.json').read_text())['matches'];byh={x['history_index']:x for x in matches};rep=next(x for x in ori['representatives'] if x['physical_bracket']==[1,2,5,6]);fo=next(x for x in fib['orbits'] if x['physical_bracket']==[1,2,5,6]);u=s.symbols('u1:9');n=8
...
total=s.factor(sum(s.sympify(x['transported_coefficient']) for x in entries));checks={'two_branches':len(entries)==2,'common_target_chart_rank_seven':len(chosen)==7,'nonzero_jacobians':all(x['target_log_jacobian']!='0' for x in entries),'nonzero_total':total!=0};out={'schema':'marici.nima.n8-1256-transported-residue-sum.v1','physical_bracket':[1,2,5,6],'target_pivot':[x+1 for x in piv],'target_coordinate_indices':chosen,'target_sample':fo['target_sample'],'contributions':entries,'summed_coefficient':str(total),'checks':checks,'passed':all(checks.values()),'claim_boundary':'This is an exact pointwise evaluation in seven ordinary normalized target coordinates. It is not yet a symbolic identity or comparison with an independent closed-form boundary canonical form.'};p=R/'research/nima/results/n8-1256-transported-residue-sum.json';p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'summed_coefficient':str(total),'checks':checks,'passed':out['passed']},indent=2));raise SystemExit(0 if out['passed'] else 1)
