...
import sympy as s
N=10; coords=s.symbols('x3:10') # fan variables X13,...,X16
...
checks={'thirtyfive_facets':len(facets)==35,'onethousandfourhundredthirty_vertices':len(vertices)==1430,'seven_facets_per_vertex':all(len(t)==7 for t in tris),'unit_jacobians':all(abs(int(r['jacobian']))==1 for r in rows),'all_other_facets_positive_at_vertices':positive,'canonical_form_equals_amplitude_termwise':bool(local_equal)}
report={'schema':'marici.nima.abhy-ten-point-associahedron-form.v1','benchmark':{'paper':'Arkani-Hamed, Bai, He, Yan, Scattering Forms and the Positive Geometry of Kinematics, Color and the Worldsheet','arxiv':'1711.09102','object':'ten-point kinematic associahedron canonical form'},'embedding':{k:str(v) for k,v in facets.items()},'positive_constants':[str(c) for c in C.values()],'oriented_dlog_vertices':rows,'vertices':vertices,'checks':checks,'passed':all(checks.values()),'scope':'Exact symbolic ten-point associahedron pullback in the fan-coordinate ABHY realization.'}
out=ROOT/'research/nima/results/abhy-ten-point-associahedron-form.json';out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'passed':report['passed'],'checks':checks},indent=2));raise SystemExit(0 if report['passed'] else 1)
