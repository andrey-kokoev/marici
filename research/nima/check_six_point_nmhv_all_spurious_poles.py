...
def check_divisor(target):
 e=s.Symbol('epsilon');base={i:s.Matrix([1,s.Integer(t),s.Integer(t)**2,s.Integer(t)**3]) for i,t in enumerate((1,2,4,7,11,16),1)}
...
checks={'three_shared_spurious_divisors':len(spurious)==3,'identified_divisors':spurious==[(1,2,4,6),(2,3,4,6),(2,4,5,6)],'all_residues_cancel':all(r['passed'] for r in runs)}
report={'schema':'marici.nima.six-point-nmhv-all-spurious-poles.v1','benchmark':{'result':'complete cancellation of internal BCFW boundaries for the six-point NMHV tree amplitude','context':'positive Grassmannian/amplituhedron triangulation'},'bcfw_terms':['[23456]','[12456]','[12346]'],'runs':runs,'total_grassmann_residues_checked':sum(r['coefficients_checked'] for r in runs),'checks':checks,'passed':all(checks.values()),'scope':'Exact transverse residue checks for every shared nonphysical four-bracket divisor of this BCFW representation.'}
out=ROOT/'research/nima/results/six-point-nmhv-all-spurious-poles.json';out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'passed':report['passed'],'divisors':[r['divisor'] for r in runs],'residues_checked':report['total_grassmann_residues_checked'],'failures':sum(r['failure_count'] for r in runs)},indent=2));raise SystemExit(0 if report['passed'] else 1)
