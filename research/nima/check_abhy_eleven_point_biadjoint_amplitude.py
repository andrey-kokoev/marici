#!/usr/bin/env python3
"""Combinatorial exact replication of the ABHY eleven-point planar amplitude."""
import json,math
from functools import lru_cache
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2];N=11
...
checks={'4862_catalan_terms':len(tris)==4862,'44_physical_channels':len(channels)==44,'eight_propagators_per_term':all(len(t)==8 for t in tris),'unit_coefficients':len(tris)==len(set(tris)),'cyclic_invariance':cyclic,'reflection_invariance':reflection,'all_44_factorization_residues':all_factor}
report={'schema':'marici.nima.abhy-eleven-point-biadjoint-amplitude.v2','benchmark':{'paper':'Arkani-Hamed, Bai, He, Yan, Scattering Forms and the Positive Geometry of Kinematics, Color and the Worldsheet','arxiv':'1711.09102','observable':'m_11[1234567891011|1234567891011]'},'representation':'Each unit-coefficient rational term is encoded exactly by its eight planar propagator diagonals.','term_count':len(tris),'physical_channels':[f'X{i}{j}' for i,j in channels],'residues':residues,'checks':checks,'passed':all(checks.values()),'scope':'Exact combinatorial rational-function identity; avoids redundant global symbolic simplification.'}
out=ROOT/'research/nima/results/abhy-eleven-point-biadjoint-amplitude.json';out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'passed':report['passed'],'term_count':len(tris),'channels':len(channels),'checks':checks},indent=2));raise SystemExit(0 if report['passed'] else 1)
