#!/usr/bin/env python3
"""Test the canonical minimal-variable-channel vertex decomposition through n=8."""
exec(compile(open(__file__.replace('check_minimal_variable_channel_vertex_decomposition.py','check_common_tree_vertex_decomposability.py'),encoding='utf-8').read().split('rows=[]')[0], 'vd-prefix', 'exec'))
...
def decompose(fs):
 ...
 mins=sorted((v for v in variable if not any(set(w)<set(v) for w in variable if w!=v)),key=lambda q:(len(q),tuple(sorted(q))))
 ...
 if not candidates:
  cache[k]=False;reasons[k]={'minimal_channels':[sorted(v) for v in mins],'reason':'no minimal channel is shedding'}
  terminal_failures.append({'facet_count':len(fs),'facets':[[sorted(q) for q in sorted(T,key=lambda q:(len(q),tuple(sorted(q))))] for T in fs],'minimal_channels':[sorted(v) for v in mins]})
  return False
