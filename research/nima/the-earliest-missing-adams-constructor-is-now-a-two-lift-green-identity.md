...
- primitive incidence at (L) with coefficient (p^{-1/2-sigma-it});
...
[
L_{P,p}:mathcal H_{mathrm{hist},p}	omathcal P_{sigma,p},
qquad
L_{Q,p}:mathcal H_{mathrm{hist},p}	omathcal Q_p
]
...
converges uniformly for (sigmage0) whenever (	heta<1/2).
...
The irreducible hostiles are now:
...
- finite lift bounds with exponent (	hetage1/2);
