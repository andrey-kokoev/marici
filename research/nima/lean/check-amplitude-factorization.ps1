...
$source = Resolve-Path (Join-Path $PSScriptRoot 'AmplitudeFactorization.lean')
$psi = [System.Diagnostics.ProcessStartInfo]::new(); $psi.FileName = 'lake'
...
if ($p.ExitCode -ne 0) {
  (($out + "`n" + $err) -split "`r?`n" | Select-Object -First 10) | ForEach-Object { Write-Output $_ }
  throw "Lean failed: $($p.ExitCode)"
}
if ($out) { Write-Output $out }
Write-Output 'MARICI_AMPLITUDE_FACTORIZATION_LEAN_OK'
