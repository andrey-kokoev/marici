...
checks={
 ...
 'arithmetic_joint_closability_constructed':False,
}
out={'schema':'marici.nima.lax-eighth-completion-coherencer.v1','kind':'lax cubical natural transformation R:Q_X=>Q_hat on axes H,V,D,q,L,C,O','components':128,'naturality_2_cells':448,'strict_face_families':['R x H','R x V','R x D','R x L','R x C (forward)','R x O'],'lax_face_family':'R x q','lax_cell':'A_X=P_X F (I-P_X)','modification_law':'A_X=P_X F (P_Y-P_X)+P_X F (I-P_Y), preserved by admitted H,V,L,O transports','dependencies':required,'checks':checks,'strict_eight_cube':False,'conditional_lax_coherencer_assembled':all(v for k,v in checks.items() if k!='arithmetic_joint_closability_constructed'),'lax_eighth_coherencer_constructed':False,'scope':['projective exponential/Laurent source completion','forward retained-graph comparison','admitted Mellin and Laurent graph multipliers','transverse marked physical cuts','source-pulled QDLO graph'],'nonclaims':['strict finite-cutoff Fourier naturality','bounded inverse to realization comparison','arbitrary completed observers','nontransverse loaded divisors']}
out['passed']=out['conditional_lax_coherencer_assembled'] and not out['lax_eighth_coherencer_constructed']
