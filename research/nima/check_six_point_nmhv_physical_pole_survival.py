#!/usr/bin/env python3
"""Exact census: every physical six-point NMHV pole has nonzero residue."""
...
def check(target):
 e=s.Symbol('epsilon');Z={i:s.Matrix([1,t,t*t,t**3]) for i,t in enumerate(map(s.Integer,(1,2,4,7,11,16)),1)}
...
checks={'nine_physical_poles':len(physical_poles)==9,'every_physical_pole_survives':all(r['passed'] for r in runs)}
report={'schema':'marici.nima.six-point-nmhv-physical-pole-survival.v1','benchmark':{'result':'only physical boundaries survive after summing the six-point NMHV BCFW triangulation','context':'amplituhedron locality from boundary geometry'},'physical_poles':['<'+''.join(map(str,q))+'>' for q in physical_poles],'runs':runs,'total_grassmann_residues_checked':sum(r['coefficients_checked'] for r in runs),'checks':checks,'passed':all(checks.values()),'scope':'Exact transverse nonvanishing-residue test; this does not yet identify each residue with a normalized product of subamplitudes.'}
out=ROOT/'research/nima/results/six-point-nmhv-physical-pole-survival.json';out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'passed':report['passed'],'physical_poles':len(runs),'residues_checked':report['total_grassmann_residues_checked'],'nonzero_components':[r['nonzero_total_residue_components'] for r in runs]},indent=2));raise SystemExit(0 if report['passed'] else 1)
