...
out={'schema':'marici.nima.nnmhv-kernel-wall-tail-boundary.v1','n':n,'kernel_matrix':[[str(s.factor(K[i,j])) for j in range(m)] for i in range(m)],'kernel_without_upper_boundary_transport':[[str(s.factor(K0[i,j])) for j in range(m)] for i in range(m)],'scalar_full':str(total(K)),'scalar_without_upper_boundary_transport':str(total(K0)),'boundary_correction_scalar':str(total(B)),'block_summary':summary,'boundary_correction_frobenius_norm':norm(B),'checks':checks,'passed':all(checks.values()),'conclusion':'Double augmentation sees only PKP. Boundary transport is diagonal in endpoint coordinates but changes both the wall channel and unresolved sectors; it is not identical to the zero-augmentation tail.'}
