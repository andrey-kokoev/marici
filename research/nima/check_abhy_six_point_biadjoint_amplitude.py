#!/usr/bin/env python3
"""Exact ABHY six-point planar biadjoint amplitude replication."""
...
verts=tuple(range(1,N+1)); m6,tris=amplitude(verts)
diagonals=sorted(set().union(*tris)); terms=sp.Add.make_args(sp.expand(m6))
...
cyclic_ok=sp.simplify(m6.xreplace(rho)-m6)==0
reflection_ok=sp.simplify(m6.xreplace(sigma)-m6)==0
...
for i,j in diagonals:
 ...
 x=var((i,j)); got=sp.simplify(sp.limit(x*m6,x,0)); ok=sp.simplify(got-expected)==0
 residue_ok &= ok
...
report={'schema':'marici.nima.abhy-six-point-biadjoint-amplitude.v1','benchmark':{'paper':'Arkani-Hamed, Bai, He, Yan, Scattering Forms and the Positive Geometry of Kinematics, Color and the Worldsheet','arxiv':'1711.09102','observable':'m_6[123456|123456]'},'convention':'overall coupling and sign stripped; X_ij are planar propagator variables','formula':str(m6),'term_count':len(terms),'physical_channels':[str(var(d)) for d in diagonals],'checks':{'fourteen_catalan_terms':len(terms)==14,'nine_physical_channels':len(diagonals)==9,'cyclic_invariance':bool(cyclic_ok),'reflection_invariance':bool(reflection_ok),'all_nine_factorization_residues':bool(residue_ok)},'residues':residues,'passed':bool(len(terms)==14 and len(diagonals)==9 and cyclic_ok and reflection_ok and residue_ok),'scope':'Exact symbolic replication of the tree-level six-point planar biadjoint scalar benchmark.'}
out=ROOT/'research/nima/results/abhy-six-point-biadjoint-amplitude.json';out.parent.mkdir(exist_ok=True);out.write_text(json.dumps(report,indent=2)+'\n')
