#!/usr/bin/env python3
"""Nine-point NMHV reflection invariance on generic exact twistor data."""
import itertools,json,random,sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2];sys.path.insert(0,str(ROOT/'research/benincasa/.tmp_sympy'))
import sympy as s
N=9;bcfw=tuple((N,i-1,i,j-1,j) for i in range(2,N-1) for j in range(i+2,N));reflected=tuple(tuple(N+1-i for i in q) for q in bcfw)
