...
import sympy as s
N=7; coords=s.symbols('x3:7') # fan variables X13,...,X16
...
checks={'fourteen_facets':len(facets)==14,'fortytwo_vertices':len(vertices)==42,'four_facets_per_vertex':all(len(t)==4 for t in tris),'unit_jacobians':all(abs(int(r['jacobian']))==1 for r in rows),'all_other_facets_positive_at_vertices':positive,'canonical_form_equals_amplitude_termwise':bool(local_equal)}
report={'schema':'marici.nima.abhy-seven-point-associahedron-form.v1','benchmark':{'paper':'Arkani-Hamed, Bai, He, Yan, Scattering Forms and the Positive Geometry of Kinematics, Color and the Worldsheet','arxiv':'1711.09102','object':'seven-point kinematic associahedron canonical form'},'embedding':{k:str(v) for k,v in facets.items()},'positive_constants':[str(c) for c in C.values()],'oriented_dlog_vertices':rows,'vertices':vertices,'checks':checks,'passed':all(checks.values()),'scope':'Exact symbolic seven-point associahedron pullback in the fan-coordinate ABHY realization.'}
out=ROOT/'research/nima/results/abhy-seven-point-associahedron-form.json';out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'passed':report['passed'],'checks':checks},indent=2));raise SystemExit(0 if report['passed'] else 1)
