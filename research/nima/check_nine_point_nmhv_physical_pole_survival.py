#!/usr/bin/env python3
"""Exact census: every physical nine-point NMHV pole has nonzero residue."""
import itertools,json,sys
from collections import Counter
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2];sys.path.insert(0,str(ROOT/'research/benincasa/.tmp_sympy'))
import sympy as s
N=9;terms=tuple((N,i-1,i,j-1,j) for i in range(2,N-1) for j in range(i+2,N))
...
def check(target):
 e=s.Symbol('epsilon');Z={i:s.Matrix([1,t,t*t,t**3]) for i,t in enumerate(map(s.Integer,(1,2,4,7,11,16,22,29,37)),1)}
...
checks={'twentyseven_physical_poles':len(physical_poles)==27,'every_physical_pole_survives':all(r['passed'] for r in runs)}
report={'schema':'marici.nima.nine-point-nmhv-physical-pole-survival.v1','benchmark':{'result':'only physical boundaries survive after summing the nine-point NMHV BCFW triangulation','context':'amplituhedron locality from boundary geometry'},'physical_poles':['<'+''.join(map(str,q))+'>' for q in physical_poles],'runs':runs,'total_grassmann_residues_checked':sum(r['coefficients_checked'] for r in runs),'checks':checks,'passed':all(checks.values()),'scope':'Exact transverse nonvanishing-residue test; this does not yet identify each residue with a normalized product of subamplitudes.'}
out=ROOT/'research/nima/results/nine-point-nmhv-physical-pole-survival.json';out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'passed':report['passed'],'physical_poles':len(runs),'residues_checked':report['total_grassmann_residues_checked'],'nonzero_components':[r['nonzero_total_residue_components'] for r in runs]},indent=2));raise SystemExit(0 if report['passed'] else 1)
