#!/usr/bin/env python3
"""Test the canonical maximal-variable-channel vertex decomposition through n=8."""
exec(compile(open(__file__.replace('check_maximal_variable_channel_vertex_decomposition.py','check_common_tree_vertex_decomposability.py'),encoding='utf-8').read().split('rows=[]')[0], 'vd-prefix', 'exec'))
...
def decompose(fs):
 ...
 mins=sorted((v for v in variable if not any(set(v)<set(w) for w in variable if w!=v)),key=lambda q:(len(q),tuple(sorted(q))))
 ...
 if not candidates:
  cache[k]=False;reasons[k]={'maximal_channels':[sorted(v) for v in mins],'reason':'no maximal channel is shedding'}
  terminal_failures.append({'facet_count':len(fs),'facets':[[sorted(q) for q in sorted(T,key=lambda q:(len(q),tuple(sorted(q))))] for T in fs],'maximal_channels':[sorted(v) for v in mins]})
  return False
