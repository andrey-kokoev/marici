...
# This file was *autogenerated* from the file /mnt/c/Users/andrey/src/marici/research/nima/check_loop_site_valence_falloff.sage
...
def main():
    ...
    bubble = graph_integrand([x1, x2], [(_sage_const_0 , _sage_const_1 , ya), (_sage_const_0 , _sage_const_1 , yb)])
    published_bubble = (
        _sage_const_1  / ((x1+x2)*(x1+ya+yb)*(x2+ya+yb))
        * (_sage_const_1 /(x1+x2+_sage_const_2 *ya) + _sage_const_1 /(x1+x2+_sage_const_2 *yb))
    )
    assert bubble == published_bubble
    assert degree_gap(bubble, ring.gen(_sage_const_0 )) == _sage_const_3 
    assert degree_gap(bubble, ring.gen(_sage_const_1 )) == _sage_const_3 
    print("parallel-edge source formula match: PASS")
    print("parallel-edge vertex falloff: x^-3")

    triangle = graph_integrand(
        [x1, x2, x3],
        [(_sage_const_0 , _sage_const_1 , ya), (_sage_const_1 , _sage_const_2 , yb), (_sage_const_2 , _sage_const_0 , yc)],
    )
    triangle_gaps = [degree_gap(triangle, ring.gen(index)) for index in range(_sage_const_3 )]
    assert triangle_gaps == [_sage_const_3 , _sage_const_3 , _sage_const_3 ]
