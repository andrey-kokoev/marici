#!/usr/bin/env python3
"""Combinatorial exact replication of the ABHY thirteen-point planar amplitude."""
import json,math
from functools import lru_cache
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2];N=13
...
checks={'58786_catalan_terms':len(tris)==58786,'65_physical_channels':len(channels)==65,'ten_propagators_per_term':all(len(t)==10 for t in tris),'unit_coefficients':len(tris)==len(set(tris)),'cyclic_invariance':cyclic,'reflection_invariance':reflection,'all_65_factorization_residues':all_factor}
report={'schema':'marici.nima.abhy-thirteen-point-biadjoint-amplitude.v2','benchmark':{'paper':'Arkani-Hamed, Bai, He, Yan, Scattering Forms and the Positive Geometry of Kinematics, Color and the Worldsheet','arxiv':'1711.09102','observable':'m_13[12345678910111213|12345678910111213]'},'representation':'Each unit-coefficient rational term is encoded exactly by its ten planar propagator diagonals.','term_count':len(tris),'physical_channels':[f'X{i}{j}' for i,j in channels],'residues':residues,'checks':checks,'passed':all(checks.values()),'scope':'Exact combinatorial rational-function identity; avoids redundant global symbolic simplification.'}
out=ROOT/'research/nima/results/abhy-thirteen-point-biadjoint-amplitude.json';out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'passed':report['passed'],'term_count':len(tris),'channels':len(channels),'checks':checks},indent=2));raise SystemExit(0 if report['passed'] else 1)
