#!/usr/bin/env python3
"""Exact reflection invariance of the twelve-point NMHV BCFW representation."""
...
def reflect(q):return tuple(N+1-i for i in q)
reflected=tuple(reflect(q) for q in bcfw)
configs=[(1,2,4,7,11,16,22,29,37,46,56,67),(0,1,3,8,14,23,35,50,68,89,113,140),(-4,-2,1,5,12,20,31,45,62,82,105,131)]
def evaluate(ts):
 ...
 data={q:five(q) for q in set(bcfw+reflected)};fails=[]
 for mono in itertools.product(range(1,N+1),repeat=4):
  def coeff(q):
   num,den=data[q];return s.prod(num.get(i,0) for i in mono)/den
  delta=s.factor(sum(coeff(q) for q in bcfw)-sum(coeff(q) for q in reflected))
...
report={'schema':'marici.nima.twelve-point-nmhv-bcfw-reflection-invariance.v1','benchmark':{'result':'reflection invariance of the twelve-point NMHV tree ratio function in momentum-twistor BCFW form','reference':'Arkani-Hamed et al., The All-Loop Integrand For Scattering Amplitudes in Planar N=4 SYM','arxiv':'1008.2958'},'bcfw_terms':['['+''.join(map(str,q))+']' for q in bcfw],'reflected_image_terms':['['+''.join(map(str,q))+']' for q in reflected],'term_count':len(bcfw),'exact_positive_configurations':runs,'total_grassmann_coefficients_checked':sum(r['coefficients_checked'] for r in runs),'passed':all(r['passed'] for r in runs),'scope':'Exact rational verification on three positive twelve-twistor configurations.'}
out=ROOT/'research/nima/results/twelve-point-nmhv-bcfw-reflection-invariance.json';out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'passed':report['passed'],'terms':len(bcfw),'configurations':len(runs),'coefficients_checked':report['total_grassmann_coefficients_checked'],'failures':sum(r['failure_count'] for r in runs)},indent=2));raise SystemExit(0 if report['passed'] else 1)
