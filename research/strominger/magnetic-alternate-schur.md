...
where (arepsilon_g) is the explicitly tracked row/column permutation sign.
...
This is an exact finite fixed-width reduction through grade (30), not yet a
closed formula for (	au_g) or an unbounded proof of (det A_g\ne0).  The
next target is a grade-(2) recurrence or product formula for these two scalar
sequences.
