...
q=[tuple(map(D,b)) for b in jet['normalized_boundary_margin_coefficients_through_degree_twenty_six']]
A=D('12.2291447802683980216431793240891324860028232975681928967813222103417342289462238562065712')
anchor=D(81); R=D(28); target=D(21)
def deriv_upper(k):
 total=D(0)
 for n in range(k,7):
  fall=D(1)
...
remainder=up.divide(up.multiply(M,up.power(r,27)),down.subtract(D(1),r))
poly0=down.divide(q[0][0],D(4)); variation=D(0)
for n in range(1,27): variation=up.add(variation,up.multiply(max(abs(q[n][0]),abs(q[n][1]))/D(4),up.power(target,n)))
poly_lower=down.subtract(poly0,variation); final=down.subtract(poly_lower,remainder)
assert den>0 and final>0
out={'source_cauchy_radius':str(R),'target_disk_radius':str(target),'normalized_source_upper_on_cauchy_radius':str(C),'normalized_source_modulus_lower_on_cauchy_radius':str(den),'Fprime_modulus_upper_on_cauchy_radius':str(M),'degree_twenty_six_polynomial_real_lower_on_target_disk':str(poly_lower),'degree_twenty_seven_and_higher_remainder_upper':str(remainder),'Fprime_real_lower_on_radius_twenty_one':str(final),'Im_F_strictly_positive_on_upper_radius_twenty_one_disk':True,'directed_decimal_rounding':True,'zero_locations_used':False,'rh_proved':False}
if __name__=='__main__':
 pth=ROOT/'results'/'degree-twenty-six-Fprime-cauchy-radius-sixteen-certificate.json';pth.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
