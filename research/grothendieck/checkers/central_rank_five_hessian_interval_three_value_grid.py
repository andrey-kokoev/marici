"""Directed Hessian audit for every central anchor with exactly three values."""
import json
...
ANCHORS = [x for x in combinations_with_replacement(GRID, 5) if len(set(x)) == 3]
WORKERS = 6
...
def main():
    ...
    output = Path(__file__).parents[1] / 'results' / 'central-rank-five-hessian-interval-three-value-grid.json'
