"""Certified degree-seven series and the three order-three Hausdorff determinants."""
import itertools,json,math
from fractions import Fraction
from pathlib import Path
from eta_order_eight_decimal_interval import totals as eta8
from quarter_point_order_two_interval import (add,sub,mul,scale,divp,powi,qbox,
                                               zeta_integer,pi,logpi)

D=7
zetas={p:(zeta_integer(p)[0][0],zeta_integer(p)[1][1]) for p in range(2,D+2)}
L=eta8[0]
eta=[scale(Fraction(1,math.factorial(k)),eta8[k]) for k in range(D+2)]
etap=[scale(Fraction(k+1),eta[k+1]) for k in range(D+1)]
q=[]
for n in range(D+1):
    numerator=etap[n]
...
ell=[]
for n in range(D+1):
    inv=qbox(Fraction((-1)**n));B=qbox(Fraction(0))
    bernoulli={0:Fraction(1,2),1:Fraction(-1,12),3:Fraction(1,720),
               5:Fraction(-1,30240),7:Fraction(1,1209600)}
    if n in bernoulli:B=scale(bernoulli[n],powi(L,n+1))
    if n==0:
        gamma=add(scale(Fraction(-1,2),g0),scale(Fraction(-1),L))
...
# epsilon=(-1+sqrt(1+4h))/2: alternating Catalan coefficients.
E=[qbox(Fraction(x)) for x in (0,1,-1,2,-5,14,-42,132)]
...
ordinary=[[A[i+j] for j in range(4)] for i in range(4)]
lower=[[A[i+j+1] for j in range(4)] for i in range(4)]
upper=[[sub(scale(Fraction(4),A[i+j]),A[i+j+1]) for j in range(4)] for i in range(4)]
...
result={'moments_A0_through_A7':[strings(x) for x in A],
        'order_three_determinant_intervals':[strings(x) for x in dets],
        'signs':signs,'interval_certified':True,'zero_locations_used':False,'rh_proved':False}
if __name__=='__main__':
    output=Path(__file__).parents[1]/'results'/'quarter-point-order-three-interval.json'
