...
out={'schema':'marici.high-precision-p262144-escape-scout.v1','certified':False,'decimal_digits':50,'prefix':P,'gauss_hermite_nodes':96,'rows':rows,'limitations':['finite xi grid','fixed Gauss-Hermite quadrature is not interval enclosed','frontier beyond each xi_max is not tested']}
(Path(__file__).parents[1]/'results'/'high-precision-p262144-escape-scout.json').write_text(json.dumps(out,indent=2)+'\n')
