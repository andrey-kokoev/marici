"""Exact parameter-uniform interval polynomial for scaled Chart-2 H and J."""
...
q0,q1=F(7,16),F(1,2);boxes=64;records=[]
for k in range(boxes):
 ...
 records.append({"q":[str(ql),str(qh)],"H_hi":float(H.hi),"J_lo":float(J.lo),
  "accepted":H.hi < -1645 and J.lo > F(5325,1000)})
result={"scope":"sixty-four disjoint local-q exact degree-13 scaled polynomial enclosures","q_boxes":boxes,
 "accepted":sum(x["accepted"] for x in records),"records":records}
out=json.dumps(result,indent=2)+"\n";Path("research/grothendieck/results/theta-chart2-scaled-taylor-q-cover.json").write_text(out);print(out,end="")
