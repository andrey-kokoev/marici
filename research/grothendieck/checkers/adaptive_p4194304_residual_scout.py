...
src=json.loads((Path(__file__).parents[1]/'results'/'high-precision-p1048576-escape-scout.json').read_text())
XS=np.array([float(x) for x in src['rows'][0]['bad_xi']])
def pp(n):
...
out={'schema':'marici.adaptive-p4194304-residual-scout.v1','certified':False,'t':T,'prefix':P,'input_candidate_count':len(XS),'remaining_count':len(bad),'remaining_xi':bad.tolist(),'last_remaining_xi':float(max(bad)) if len(bad) else None,'minimum_guarded_margin':float(min(margin)),'U0':U0,'U2':U2,'float64_guard':GUARD,'limitations':['tests only prior grid candidates','digamma quadrature not interval enclosed','finite grid']};(Path(__file__).parents[1]/'results'/'adaptive-p4194304-residual-scout.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({k:out[k] for k in ('input_candidate_count','remaining_count','last_remaining_xi','minimum_guarded_margin','U0','U2')},indent=2))
