"""Directed Hessian audit for every central anchor with at most two values."""
import json
...
ANCHORS = [x for x in combinations_with_replacement(GRID, 5) if len(set(x)) <= 2]
...
def main():
    ...
    output = Path(__file__).parents[1] / 'results' / 'central-rank-five-hessian-interval-two-value-grid.json'
