...
P = 65536
SLICES = [
    (mp.mpf('2'), mp.mpf('150'), mp.mpf('.5')),
    (mp.mpf('3'), mp.mpf('700'), mp.mpf('1')),
]
...
out={'schema':'marici.high-precision-large-prefix-escape-scout.v1','certified':False,'decimal_digits':50,'prefix':P,'gauss_hermite_nodes':96,'rows':rows,'limitations':['finite xi grid','fixed Gauss-Hermite quadrature is not interval enclosed','frontier beyond each xi_max is not tested']}
(Path(__file__).parents[1]/'results'/'high-precision-large-prefix-escape-scout.json').write_text(json.dumps(out,indent=2)+'\n')
