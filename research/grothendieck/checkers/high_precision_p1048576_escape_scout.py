...
for t,xmax,step in SLICES:
    ...
    while x<=xmax:
        ...
        x+=step
    rows.append({'t':mp.nstr(t,15),'xi_step':mp.nstr(step,15),'xi_max':mp.nstr(xmax,15),'bad_count':len(bad),'bad_frontier':mp.nstr(max((r[1] for r in bad),default=mp.nan),20),'bad_xi':[mp.nstr(r[1],20) for r in bad],'worst_margin':mp.nstr(worst[0],30),'worst_xi':mp.nstr(worst[1],20),'worst_rr':mp.nstr(worst[2],30),'worst_ii':mp.nstr(worst[3],30),'U0':mp.nstr(U0,30),'U2':mp.nstr(U2,30)})
    print(json.dumps(rows[-1]))
out={'schema':'marici.high-precision-p1048576-escape-scout.v1','certified':False,'decimal_digits':50,'prefix':P,'gauss_hermite_nodes':96,'rows':rows,'limitations':['finite xi grid','fixed Gauss-Hermite quadrature is not interval enclosed','frontier beyond each xi_max is not tested']}
(Path(__file__).parents[1]/'results'/'high-precision-p1048576-escape-scout.json').write_text(json.dumps(out,indent=2)+'\n')
