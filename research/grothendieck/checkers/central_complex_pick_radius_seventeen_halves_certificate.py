"""Directed elementary-anchor certificate of Pick positivity on |t|<=17/2."""
import json
...
# At t=81, s=19/2. Elementary bounds:
# pi^(-19/4)<1/162, Gamma(19/4)<10395/512, zeta(19/2)<21/20.
xi_nineteen_halves_upper = Q(323, 8) * Q(1, 162) * Q(10395, 512) * Q(21, 20)
center_lower = D(rouche["Xi_half_lower_bound"])
C81_upper = up.divide(D(xi_nineteen_halves_upper.numerator) / D(xi_nineteen_halves_upper.denominator), center_lower)
assert C81_upper < D(13)
anchor_upper = D(13)
anchor_radius = D(81)
radius = D("8.5")
...
assert F_prime_real_lower > D("0.008")

result = {
    "elementary_Xi_nineteen_halves_upper": str(xi_nineteen_halves_upper),
    "normalized_C81_upper": str(C81_upper),
    "tail_anchor_used": "C(81)<13",
    "complex_t_disk_radius": str(radius),
    "degree_seven_tail_C_Cprime_Cdoubleprime": [str(tail_C), str(tail_C_prime), str(tail_C_double)],
    "normalized_C_modulus_lower_on_radius_seventeen_halves": str(denominator_lower),
    ...
    "F_prime_real_part_lower_on_radius_seventeen_halves": str(F_prime_real_lower),
    "upper_half_disk_pick_margin_per_unit_height": str(F_prime_real_lower),
    "Im_F_strictly_positive_on_upper_radius_seventeen_halves_disk": True,
...
if __name__ == "__main__":
    output = ROOT / "results" / "central-complex-pick-radius-seventeen-halves-certificate.json"
