...
I=P.I; D=Decimal; P.O=25; P.DEPTH=300
...
ell_prime=[I.scale(xi_log[2*n+1],Fraction(1,2)) for n in range(12)]

P.O=11
t=[I.box(0),I.box(1)]+[I.box(0)]*10
f=P.mul(P.sub(P.scale(t,4),P.c(1)),ell_prime)
g=[I.scale(f[n+1],n+1) for n in range(11)]+[I.box(0)]
h=P.powj(g,Fraction(-1,2))

assert all(a<=0<=b for a,b in even)
result={
    'centered_q_order':25,
    'H_coefficients_through_degree_eleven':[[str(a),str(b)] for a,b in h],
...
if __name__=='__main__':
    ...
    print(f"H_coefficient_count={len(h)}")
