#!/usr/bin/env python3
"""Class-level endpoint and generic audit of the supported short-Rees maps.
...
def main_class_test(path):
    begun=time.time()
    models=source_models();C,L,phi,beta,alpha=raw_to_supported_cone(models)
    prior_info,gysin_maps=complete_central_Gysin_channels(models,C,L,phi)
    prior_records,allmaps=native_free_lifts(models,C,L,phi,gysin_maps)
    inherited_checks=sum(COUNT.values())
    W=tensor_target(L);records=[];witnesses=[]
    for index,(S,m,lift,F,A) in enumerate(allmaps):
        labels=tuple(prior_info[index]['branch_subset']);eps=prior_info[index]['eta'];n=m-2
        w=tuple(OD.index(i) for i in labels)+((4,) if eps else ())
        raw_top=tuple(sorted(w+(3,)))
        lam=ea(ea(GAMMA,ex({q:-1 for q in range(6)})),
               ea(models['split_selected'][2][w],ex({0:1,9:1})))
        ...
        dc=hom_differential(closed,S,W,n)
        for s in S[0]:
            check(not dc[s],'class_test_closed_full_cap')
            oldq={(t,e):v for (t,e),v in F[s].items() if target_side(t[0])=='Q'}
            newq={(t,e):v for (t,e),v in closed[s].items() if target_side(t[0])=='Q'}
            check(oldq==newq,'class_test_generic_unchanged_by_endpoint_homotopy')
        # All eight partial central faces preserve the COMPLETE equations.
        for center in subsets(OD):
            cc=specialize_table(closed,center)
            for s in S[0]:
                left={z:v for z,v in apply(W[1],cc[s]).items() if output_allowed(z[0],z[1],center)}
                right={z:v for z,v in scale(apply(cc,S[1][s]),pm(n)).items() if output_allowed(z[0],z[1],center)}
                check(left==right,'class_test_eight_central_faces_closed_map')
            for side,hfamily in hs.items():
                for I,h in hfamily.items():
                    if not I:continue
                    specialized=specialize_table(h,center)
                    dh=hom_differential(specialized,S,W,n-1+len(I))
                    for s in S[0]:
                        lhs={z:v for z,v in dh[s].items() if output_allowed(z[0],z[1],center)}
                        exp=add(*(scale(hfamily[I[:j]+I[j+1:]][s],pm(j)) for j in range(len(I))))
                        exp={z:v for z,v in exp.items() if output_allowed(z[0],z[1],center)}
                        check(lhs==exp,'class_test_eight_central_faces_endpoint_coherence')
        # Literal top-column formula, before homology or purity projection.
        top=tuple(range(7))
        expected={((c,('u',raw_top)),e):pm(eps+1)*v for (c,e),v in omega(()).items()}
        check(closed[top]==expected,'class_test_exact_generic_top_Koszul_column')
        check(not A[top],'class_test_endpoint_top_column_zero')
        two=int(3 in labels)+eps
        rank=2 if two==1 else 1
        # The complete regular-purity quotient is constructed first.  It
        # is tiny because all short-localization summands meet a killed X.
        pg,pd,pcf=purity_projection(S,W,lam,labels)
        ph,pproj,pcancel=fast_reduce(pg,pd,[{t:v for (t,e),v in expected.items() if t in pg}])
        check(ph=={n:rank},'class_test_regular_purity_independent_homology')
        check(all(target_side(t[0])=='Q' for t in pg),'class_test_purity_has_only_generic_rows')
        check(any(abs(v)==1 for v in pproj[0].values()),'class_test_purity_primitive_class')
        group_records=[]
        for center in [(),OD]:
            g,d,cf=target_Hom(S,W,lam,center)
            projection={x:({x[1]:1} if x[0]==top and x[1] in pg else {}) for x in g}
            for x in g:
                if projection[x]:check(cf[x]==pcf[x[1]],'class_test_purity_projection_exact_degree')
                check(lin(projection,d[x])==lin(pd,projection[x]),
                      'class_test_purity_projection_on_every_Hom_column')
            sections={}
            for kind in ['K','E','Q','B','V']:
                gg,dd=support_section(g,d,kind)
                # Restriction of the same quotient proves naturality on the
                # entire support filtration.  The quasi-isomorphism itself
                # is regular Koszul purity, proved in the accompanying note.
                if kind in ('K','E','Q'):
                    vf=table_vector(closed,cf,gg,center)
                    check(not lin(dd,vf),'class_test_closed_class_in_complete_Hom')
                    image=lin(projection,vf)
                    check(image=={t:v for (t,e),v in expected.items() if t in pg},
                          'class_test_class_is_exact_purity_generator')
                    hom=ph
                else:
                    check(not any(projection[x] for x in gg),
                          'class_test_short_support_purity_quotient_zero')
                    hom={}
                # Independent elimination for the actual generic and
                # endpoint Hom complexes (no inferred ranks for these).
                independent=None
                if index==0 and kind in ('Q','V'):
                    hh,_,npiv=reduce_units(gg,dd,False)
                    check(hh==hom,'class_test_independent_Q_and_V_Smith')
                    independent={'unit_cancellations':npiv,'homology':hh}
                sections[kind]={'columns':len(gg),'homological_homology_ranks':hom,
                                'computed_by':'natural regular-purity projection',
                                'independent_integral_reduction':independent}
            group_records.append({'central_labels':list(center),'sections':sections})
        pair_counts={}
        for side,fam in hs.items():
            totals=Counter()
            for I,h in fam.items():
                if I:totals[len(I)]+=sum(map(len,h.values()))
            pair_counts[side]={str(k):v for k,v in sorted(totals.items())}
        records.append({'odd_subset':list(labels),'eta':eps,'homological_map_degree':n,
            'fine_degree':list(lam),'rank':rank,'raw_wedge':list(raw_top),
            'full_Hom':group_records,'purity_columns':len(pg),
            'purity_unit_cancellations':pcancel,'endpoint_cohomology_classes':'both zero',
            'fixed_generic_relative_space':'contractible in this homogeneous frame',
            'endpoint_homotopy_term_counts_by_order':pair_counts})
        witnesses.append({'odd_subset':list(labels),'eta':eps,
            'endpoint_nullhomotopies':{side:{str(I):encoded_table(h) for I,h in fam.items() if I}
                                     for side,fam in hs.items()},
            'closed_cap':encoded_table(closed),'generic_top':encoded_table({top:expected})})
        print(json.dumps({'channel':list(labels),'eta':eps,'rank':rank,
                         'K_columns':group_records[0]['sections']['K']['columns'],
                         'endpoint_classes':'zero','generic':'primitive'}),flush=True)
    result={'status':'proved_for_the_eight_existing_supported_maps_in_their_prescribed_frames',
        'source_commit':COMMIT,'source_blobs':SOURCE_BLOBS,
        'inputs':{'source':'K(X0,...,X5,t_T), all 128 columns',
                  'target':'actual 215-state graph-Rees K tensor Cone(Du -> Du[tau^-1])',
                  'native_resolution_generators':50,'independent_eta':True},
        'theorem':{'generic_classes':'all eight primitive and nonzero',
                   'endpoint_composites':'each is explicitly coherently nullhomotopic',
                   'short_support_Hom':'acyclic in all eight complete specified frames',
                   'K_to_E_to_Q':'quasi-isomorphisms in those frames',
                   'same_framed_nonzero_endpoint_unit':'not an available class in these frames',
                   'full_physical_collar_identification':False,'physical_parity_assigned':False},
        'channels':records,'explicit_cochains':witnesses,
        'inherited_reconstruction_checks':inherited_checks,
        'new_checks':sum(COUNT.values())-inherited_checks,
        'total_checks':sum(COUNT.values()),'check_categories':dict(sorted(COUNT.items())),
        'proof_scope':['all coefficient domains retained; no global occurrence inversion',
                       '128-column free input, not an isolated residue representative',
                       'homogeneous components are exact graded groups, not polynomial truncations',
                       'independent regular-sequence purity proof establishes the all-replacement statement',
                       'no assertion that a homotopy to zero erases a separately specified geometric 2-cell',
                       'full Hom quasi-isomorphisms proved by natural regular purity; independent unreduced Smith checks only in the first representative frame',
                       'no raw-source unit lift, amplitude theorem, or full six-functor identification'],
        'runtime_seconds':round(time.time()-begun,3)}
    path.write_text(json.dumps(result,indent=2)+'\n')
    print(json.dumps({k:result[k] for k in ('inherited_reconstruction_checks','new_checks','total_checks','runtime_seconds')},indent=2))


if __name__=='__main__':
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--output',type=Path,default=Path('marici_supported_endpoint_class_test_certificate.json'))
    args=parser.parse_args()
    main_class_test(args.output)
