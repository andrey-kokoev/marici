...
const N: usize = 8;
const NONCOMMON_WALLS: usize = N - 1;
...
fn enumerate_terms(
    ...
    let needed = NONCOMMON_WALLS - chosen.len();
    ...
    for index in start..=facets.len() - needed {
        ...
        // A maximal C8 source term has exactly seven noncommon rows available
        // to raise the common rank from 9 to 16.  Therefore every selected row
        // must increase the quotient rank at the moment it is added.
        let partial_rows = common_rows
            .iter()
            .cloned()
            .chain(chosen.iter().map(|chosen_index| facets[*chosen_index].row.clone()))
            .chain(std::iter::once(facets[index].row.clone()))
            .collect::<Vec<_>>();
        if rank_mod(partial_rows) != common_rows.len() + chosen.len() + 1 {
            continue;
        }
        // Intersecting further facets cannot increase affine face rank.
        if affine_rank(next_mask, vertices) < N {
            continue;
        }
        chosen.push(index);
        enumerate_terms(
...
fn main() {
    ...
    let output = std::env::var("OUTPUT")
        .unwrap_or_else(|_| {
            if N == 9 {
                "../results/nine-site-full-source-inventory.json".into()
            } else {
                "../results/eight-site-full-source-inventory.json".into()
            }
        });

    let replication_mode = std::env::var("REPLICATION").ok().as_deref() == Some("1");
    let all_orbit_output = if replication_mode {
        orbit_packets.iter().map(|(_, packet)| json!({
            "canonical_key":packet["canonical_key"],
            "stabilizer_order":packet["stabilizer_order"],
            "enumerated_source_multiplicity":packet["enumerated_source_multiplicity"]
        })).collect::<Vec<_>>()
    } else {
        orbit_packets.iter().map(|(_,packet)|packet.clone()).collect::<Vec<_>>()
    };
    let packet = json!({
        "schema":if N == 9 {"marici.nine_site_full_source_inventory.v1"} else {"marici.eight_site_full_source_inventory.v1"},
        "prime":prime(),
        ...
        "selected_exact_certificate":{"integer_denominator_rank":2*N,"affine_face_rank":N},
        "all_orbits":all_orbit_output,
        ...
        "scope":format!("complete frozen {N}-site maximal source-term inventory and pre-geometric orbit ranking")
    });
    fs::write(
        output,
