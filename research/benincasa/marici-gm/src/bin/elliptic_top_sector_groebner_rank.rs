    ...
    while let Some((left, right)) = pairs.pop_front() {
        ...
        if !remainder.is_zero() {
            ...
            for old_index in 0..new_index {
                pairs.push_back((old_index, new_index));
            }
...
fn main() {
    ...
    let z2 = z * z;
    let h = x2 + y2 - z2;
    let g_a_constant = x2 * (x2 - y2 - z2);
    let g_a_c2 = -x2 + y2 - z2;
    let g_b_constant = y2 * (y2 - x2 - z2);
    let g_b_c2 = -y2 + x2 - z2;
    let h_c2 = -x2 - y2 + z2;
    let k = sum(&[
        a.power(4).scale(x2),
    ...
    let denominators = [
        Factor {
            name: "q_g1",
            polynomial: sum(&[c.clone(), b, Polynomial::constant(x)]),
            exponent: 17,
        },
        Factor {
            name: "q_g2",
            polynomial: sum(&[c.clone(), a, Polynomial::constant(y)]),
            exponent: 19,
        },
        Factor {
            name: "q_G12",
            polynomial: c.add(&Polynomial::constant(x + y + z)),
            exponent: 23,
        },
    ];
    ...
    for mask in masks {
        ...
        println!(
            "prime={} point={point} xyz=({x},{y},{z}) mask={mask:03b} factors={names:?} deletion_closed_rank={rank} basis_size={basis_size} elapsed_ms={elapsed_ms}",
            prime()
        );
    }
    if only.is_none() {
        let closed = closed_ranks.map(|rank| rank.expect("complete cube rank"));
        let mut proper = [0_usize; 8];
        ...
        assert_eq!(closed, [7, 8, 8, 9, 16, 18, 18, 21]);
        assert_eq!(proper, [7, 1, 1, 0, 9, 1, 1, 1]);
