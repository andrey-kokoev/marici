...
// Canonicalize the seven-object relational structure by all 7! permutations.
...
fn poset_signature(labels: &[String]) -> String {
    fn visit(
        ...
        // The color word precedes every relation symbol in the canonical
        // encoding.  Hence every lexicographic minimum has the remaining
        // object colors sorted; only permutations within an equal-color block
        // need to be explored.
        let color = |index: usize| {
            parse_region(&labels[index])
                .map(|region| format!("r{:02}", region.len()))
                .unwrap_or_else(|| format!("G{:02}", N))
        };
        let least_color = remaining.iter().map(|index| color(*index)).min().unwrap();
        for index in 0..remaining.len() {
            if color(remaining[index]) != least_color {
                continue;
            }
...
fn main() {
    ...
    let mut packets = classes.into_iter().map(|(key, mut members)| {
        members.sort_by(|l,r| {
            r["stabilizer_order"].as_u64().cmp(&l["stabilizer_order"].as_u64())
                .then_with(|| score(r).cmp(&score(l)))
                .then_with(|| l["canonical_key"].as_str().unwrap().cmp(r["canonical_key"].as_str().unwrap()))
        });
        let maximal_stabilizer = members.iter().map(|m|m["stabilizer_order"].as_u64().unwrap()).max().unwrap();
        let selected_score = score(&members[0]);
        let ties = members.iter().take_while(|m| {
            m["stabilizer_order"].as_u64()==Some(maximal_stabilizer) && score(m)==selected_score
        }).count();
        json!({
            "class_key":key,
            ...
            "maximal_stabilizer_order":maximal_stabilizer,
            "selected_score":{"wall_count":selected_score.0,"nesting_depth":selected_score.1,"separated_region_pairs":selected_score.2,"labelled_orbit_size":selected_score.3},
    ...
    packets.sort_by(|l, r| {
        r["free_signed_energies"]
            .as_u64()
            .cmp(&l["free_signed_energies"].as_u64())
            .then_with(|| r["maximal_stabilizer_order"].as_u64().cmp(&l["maximal_stabilizer_order"].as_u64()))
            .then_with(|| {
                l["class_key"]
                    .as_str()
                    .unwrap()
    ...
    let packet = json!({
        "schema":"marici.eight_site_inventory_partition.v1",
        "source_inventory":"eight-site-full-source-inventory.json",
        ...
        "predeclared_ranking":"visit each compatibility-poset type once before additional members; order types by maximal free loop-square rank, then nontrivial stabilizer order, then canonical class key; within a type prioritize stabilizer order, then the frozen combinatorial score, then the lexicographically least labelled key; no cover or discriminant data enter",
    ...
    fs::write(
        "../results/eight-site-inventory-partition.json",
