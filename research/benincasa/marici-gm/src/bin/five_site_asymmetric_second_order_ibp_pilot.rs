...
fn omega_jet(z:i64,u:[i64;3],roots:[i64;5],common:&[String],terms:&[Vec<String>],p:i64)
    ...
    let mut omega=0;let mut dz_omega=0;let mut dzz_omega=0;let mut du_omega=[0;3];
    for labels in all_terms{
        let mut reciprocal=1;let mut log_z=0;let mut log_z_square=0;let mut log_u=[0;3];
        for label in labels{
            let (q,qz,qu)=wall(label,z,&roots,&dy,p)?;
            let iq=inv(q,p);
            reciprocal=mul(reciprocal,iq,p);
            log_z=add(log_z,mul(qz,iq,p),p);
            log_z_square=add(log_z_square,mul(mul(qz,qz,p),mul(iq,iq,p),p),p);
        ...
        dzz_omega=add(dzz_omega,mul(reciprocal,add(mul(log_z,log_z,p),log_z_square,p),p),p);
        for i in 0..3{du_omega[i]=add(du_omega[i],-mul(reciprocal,log_u[i],p),p);}
    }
    if omega==0{None}else{Some((omega,dz_omega,dzz_omega,du_omega))}
}
...
fn test(prime:i64,z:i64,degree:usize,coordinate_relative:bool,common:&[String],terms:&[Vec<String>])->Value{
    let mons=monomials(degree);
    let fields=(0..3).flat_map(|i|mons.iter().copied().filter(move |exp|!coordinate_relative||exp[i]>0).map(move |exp|(i,exp))).collect::<Vec<_>>();
    let columns=2+fields.len();let target_points=columns+24;
    ...
    while accepted_points<target_points&&attempts<200_000{
        ...
        for sign_mask in 0_usize..32{
            ...
            let Some((omega,dz,dzz,du))=omega_jet(z,u,roots,common,terms,prime)else{continue;};
            let mut row=vec![0_i64;columns+1];row[0]=omega;
            row[0]=dz;row[1]=omega;
            for (m,(i,exp)) in fields.iter().enumerate(){
                ...
                row[2+m]=add(mul(derivative,omega,prime),mul(value,du[*i],prime),prime);
            }
            row[columns]=(-dzz).rem_euclid(prime);orbit_rows.push(row);
        }
    ...
    let no_scalar=rows.iter().map(|row|{
        let mut reduced=row[2..columns].to_vec();
        reduced.push(row[columns]);
        reduced
    }).collect::<Vec<_>>();
    let (rank_no_scalar,consistent_no_scalar)=matrix_rank(no_scalar,prime,columns-2);
    let (rank,consistent)=matrix_rank(rows,prime,columns);
    json!({"prime":prime,"z":z,"vector_degree":degree,"coordinate_relative":coordinate_relative,"unknowns":columns,"rows":row_count,"base_points":accepted_points,
        "rank":rank,"consistent":consistent,"rank_without_lower_derivatives":rank_no_scalar,
        "consistent_without_lower_derivatives":consistent_no_scalar,"lower_derivative_terms_required":consistent&&!consistent_no_scalar,"attempts":attempts})
}
...
fn main(){
    ...
    for relative in [false]{for (p,z) in [(1009,7),(1009,11),(1013,7),(1013,11)]{for degree in 0..=5{tests.push(test(p,z,degree,relative,&common,&terms));}}}
    let packet=json!({"schema":"marici.benincasa.five_site.asymmetric.second_order_ibp_pilot.v1",
        "ansatz":"d_z^2 Omega + a_1(z)d_z Omega + a_0(z)Omega = sum_i d_ui(V_i Omega), V_i polynomial in u of total degree <=d",
        "tests":tests,
    ...
    fs::write("../results/five-site-asymmetric-second-order-ibp-pilot.json",serde_json::to_string_pretty(&packet).unwrap()+"\n").unwrap();
