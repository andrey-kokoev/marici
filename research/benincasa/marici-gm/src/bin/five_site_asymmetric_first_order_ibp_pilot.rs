    ...
    while accepted_points<target_points&&attempts<200_000{
        ...
        for sign_mask in 0_usize..32{
            ...
            for (m,(i,exp)) in fields.iter().enumerate(){
                ...
                row[1+m]=add(mul(derivative,omega,prime),mul(value,du[*i],prime),prime);
            }
...
fn main(){
    ...
    let packet=json!({"schema":"marici.benincasa.five_site.asymmetric.first_order_ibp_pilot.v1",
        "ansatz":"d_z Omega + a(z) Omega = sum_i d_ui(V_i Omega), V_i polynomial in u of total degree <=d; coordinate_relative requires u_i divides V_i",
        "tests":tests,
    ...
    fs::write("../results/five-site-asymmetric-first-order-ibp-pilot.json",serde_json::to_string_pretty(&packet).unwrap()+"\n").unwrap();
