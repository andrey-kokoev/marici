...
fn main(){
 ...
 for d in [12u8,16,20,24,28]{
  ...
  let pos:HashMap<M,usize>=mons.iter().enumerate().map(|(i,m)|(*m,i)).collect();let base:Vec<M>=mons.iter().copied().filter(|m|m.u==0&&m.a%2==1).collect();let bpos:HashMap<(u8,u8),usize>=base.iter().enumerate().map(|(i,m)|((m.a,m.b),i)).collect();let mut pairs_odd:Vec<(Vec<u64>,Vec<u64>)>=Vec::new();let mut cols=Vec::new();let mut minus_cols=Vec::new();
  for(sa,sb)in[(1,1),(1,0),(0,1),(0,0)]{for s in 0..=d{for i in 0..=s{let f=mono(0,i,s-i,1);for plus in[false,true]{for q in[false,true]{let p=exact(sa,sb,&f,q,plus,&k,&l1,&l2m,&l2p);if p.is_empty()||p.keys().any(|m|m.a+m.b>d){continue}let mut v=vec![0;mons.len()];let mut uv=vec![0;mons.len()];let mut v0=vec![0;base.len()];let mut v1=vec![0;base.len()];for(m,c0)in &p{v[*pos.get(m).unwrap()]=*c0;if m.u==0{uv[*pos.get(&M{u:1,a:m.a,b:m.b}).unwrap()]=*c0}if m.a%2==1{let j=*bpos.get(&(m.a,m.b)).unwrap();if m.u==0{v0[j]=*c0}else{v1[j]=*c0}}}pairs_odd.push((v0,v1));if !plus{minus_cols.push(v.clone());minus_cols.push(uv.clone())}cols.push(v);cols.push(uv)}}}}}
