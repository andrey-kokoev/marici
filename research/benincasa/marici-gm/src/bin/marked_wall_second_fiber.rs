...
fn geometry(uu: u64, vv: u64, axis: char) -> Geometry {
    ...
    let l1 = Poly::mon(0, 1, one).add(&Poly::c(x.x.sub(u.x)));
    let l2 = Poly::mon(1, 0, one).add(&Poly::c(v.x.sub(u.x).mul(half).sub(x.x)));
    Geometry {
        k,
        kp,
        k1,
...
fn main() {
    ...
    assert_eq!(jet(1, 9).0, frac(-3, 40));
    assert_eq!(jet(1, 10).0, frac(1, 140));
    assert_eq!(jet(1, 11).0, frac(-1, 140));
    assert_eq!(jet(2, 0).0, F::z());
    assert_eq!(jet(2, 9).0, frac(3, 40));
    assert_eq!(jet(2, 10).0, frac(-1, 140));
    assert_eq!(jet(2, 11).0, frac(1, 140));
    ...
    println!("{{");
    println!("  \"schema\": \"marici.benincasa.marked_wall_second_fiber.v1\",");
    println!("  \"prime\": {},", P);
    println!("  \"path\": \"x=2,y=5,u=E,v=14-E\",");
    println!("  \"samples\": 80,");
    ...
    println!("  \"scope\": \"canonical seven-coordinate projection at the independent x=2,y=5 physical wall fiber\"");
    println!("}}");
}
