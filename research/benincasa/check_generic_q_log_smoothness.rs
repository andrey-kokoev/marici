//! Exact generic-Q audit for the frozen q_G12 marked residue of ledger 161.
//!
...
impl Checker {
    ...
    fn reject_q_factor(&mut self, category: &'static str, name: &str, polynomial: &Poly, q: &Poly) {
        self.condition(
        ...
        let remainder = pseudo_remainder(polynomial, q, Z).primitive_part();
        self.condition(
            &format!("irreducible Q is not a factor of {category}/{name}"),
            !remainder.is_zero(),
        );
        self.q_rejections += 1;
        *self.categories.entry(category).or_insert(0) += 1;
        if self.emitted {
            println!(
                "POLYNOMIAL category={category} name={name} P=({}) Qrem=({remainder})",
...
fn main() {
    ...
    for (name, polynomial) in surface_conditions {
        check.reject_q_factor("surface_singularity", name, &polynomial, &q);
    }
    ...
    for (name, polynomial) in surface_codimension_one_factors {
        check.reject_q_factor("surface_codimension_one_factor", name, &polynomial, &q);
    }
    ...
    for (name, polynomial) in base_conditions {
        check.reject_q_factor("base_or_soft_boundary", name, &polynomial, &q);
    }
    ...
    for line_index in 0..lines.len() {
        ...
        if !condition.is_constant() {
            check.reject_q_factor("line_branch_tangency", line.name, &condition, &q);
        }
    }
    check.condition(
    ...
    println!("GENERIC-Q RAW LOG-SMOOTHNESS AUDIT: EXACT CHECKS PASS");
    ...
    println!("Q_component_rejections={}", check.q_rejections);
