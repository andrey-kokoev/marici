...
X1=3*lambda, X2=lambda, X3=1.  It derives, rather than imports, the
...
def _write_uncaught(exc_type, exc, tb):
    Path(__file__).with_name("nine_master_second_slice_connection.error.txt").write_text(
...
result={
 "schema":"marici.benincasa.nine_master_second_slice_connection.v1",
 "slice":{"X1":"3*lambda","X2":"lambda","X3":"1","E":str(E)},
...
out=Path(__file__).with_name("nine_master_second_slice_connection.json")
