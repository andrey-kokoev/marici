#!/usr/bin/env python3
"""Exact Griffiths--Dwork certificate for the q_G12 nine-master residue basis.

This deliberately uses the generic transverse homogeneous slice
X1=2*lambda, X2=lambda, X3=1.  It derives, rather than imports, the
connection of the residue classes of arXiv:2408.16386v2 eq. (58).
"""
import json
...
def _write_uncaught(exc_type, exc, tb):
    Path(__file__).with_name("nine_master_slice_connection.error.txt").write_text(
...
published_p=sp.factor(
    (5*9*la**4-6*5*la**2+1)/(9*la**5-10*la**3+la)
)
published_q=sp.factor((27*la**2-10)/(9*la**4-10*la**2+1))
pf_matches_published=sp.factor(pf_p-published_p)==0 and sp.factor(pf_q-published_q)==0

result={
 "schema":"marici.benincasa.nine_master_slice_connection.v1",
 "slice":{"X1":"2*lambda","X2":"lambda","X3":"1","E":str(E)},
 ...
 "published_L2_match":pf_matches_published,
...
out=Path(__file__).with_name("nine_master_slice_connection.json")
