#!/usr/bin/env python3
"""Test jet closure after adjoining the square-free degree-two layer on all five labelled poles."""
...
def strict_evaluate(raw,pres,free_set):
    ...
    if forbidden:
        ...
        raise RuntimeError("ten-class degree-two Boolean adapter is not jet-closed")
...
jet.OUT=BEN/"results"/f"rank36-degree2-boolean-adapter-source-{CHART.lower()}-p{P}.json"
try:
...
packet={
 "schema":"marici.rank36-degree2-boolean-adapter-closure.v1",
 "prime":P,"chart":CHART,
 ...
 "error":error,
 "checks":{
   "all_ten_square_free_degree_two_classes_adjoined":True,
   "no_ambient_truncation_performed":True,
   "candidate_has_expected_dimension":state["candidate_dimension"]==36,
 },
 "passed":state["candidate_dimension"]==36,
 "conclusion":(
   "The ten-class degree-two Boolean adapter is closed under the tested source covariant jets."
   if outcome=="closed" else
   "The ten-class degree-two Boolean adapter is not closed; the first forbidden residual identifies the next target grade."
 )
}
out=BEN/"results"/f"rank36-degree2-boolean-adapter-{CHART.lower()}-p{P}.json"
