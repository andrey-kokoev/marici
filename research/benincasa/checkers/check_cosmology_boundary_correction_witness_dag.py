...
def main():
 ...
 s=source['records'][0];kp=s['k_pole'];i,j=s['exponent'];t={(x['k_pole'],*x['exponent']):x for x in target['records']}[(kp,i,j+2)];row={}
 for term in s['terms']:
 ...
 payload={'target_deps':target_deps,'nodes':{str(n):nodes[n] for n in sorted(closure)}};digest=hashlib.sha256(json.dumps(payload,sort_keys=True,separators=(',',':')).encode()).hexdigest();counts={f:sum(nodes[n]['family']==f for n in closure) for f in ('T','K')}
 out={'schema':'marici.benincasa.cosmology-boundary-correction-witness-dag.v1','field':base.PRIME,'from_ambient':12,'to_ambient':14,'source_record_index':0,'source_label':{'k_pole':kp,'exponent':[i,j]},'target_exponent':[i,j+2],'raw_difference_support':raw_support,'basis_row_count':len(basis),'basis_node_count':len(nodes),'closure_node_count':len(closure),'closure_family_counts':counts,'target_reduction_steps':len(target_deps),'dag_sha256':digest,'payload':payload,'exact_zero_replay':True,'source_natural_formula_extracted':False,'passed':True};(R/'cosmology_boundary_correction_witness_dag.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({k:out[k] for k in ('source_label','target_exponent','raw_difference_support','closure_node_count','closure_family_counts','target_reduction_steps','dag_sha256','passed')},indent=2))
