...
VRES=ROOT/'research'/'voevodsky'/'results';BRES=ROOT/'research'/'benincasa'/'results';A=rees.AMBIENT;OUT=BRES/f'cosmology_physical_half_twist_q_template_base_closure_census_a{A}.json';P=base.PRIME;charts.GAMMA=-pow(2,-1,P)%P
def add(row,c,v):
 x=(row.get(c,0)+v)%P
 if x:row[c]=x
 else:row.pop(c,None)
...
def main():
 ...
 for kp in (0,1):
  ...
  for s in base.monomials_at_most(A-7):
   ...
   records.append({'shift':list(s),'source_rows':len(source),'T_rows':sum(k[0]=='T' for k in source),'S_K_rows':sum(k[0]=='S_K' for k in source),'retained_columns':len(cols),'active_nodes':active,'edges':edges,'maximum_depth':depth,'source_generators':[{'kind':k[0],'row_index':k[1]} for k in sorted(source)],'retained_column_indices':sorted(cols)})
  results[f'k{kp}']={'shifts':len(records),'source_rows_min':min(r['source_rows'] for r in records),'source_rows_max':max(r['source_rows'] for r in records),'columns_min':min(r['retained_columns'] for r in records),'columns_max':max(r['retained_columns'] for r in records),'depth_max':max(r['maximum_depth'] for r in records),'records':records,'maximal_records':[r for r in records if r['source_rows']==max(x['source_rows'] for x in records)]}
 out={'schema':'marici.benincasa.cosmology-physical-half-twist-q-template-base-closure-census.v1','status':'physical_half_twist_all_interior_base_dependency_closures_censused','physical_gamma':'-1/2','field':P,'ambient_relation_degree':A,'results':results,'decision':'All corrected targets have bounded source closures suitable for exact rational minor solves.','limitations':['closure selected by p=32003 pivot order','census is not exact rational verification'],'passed':True}
