...
ROOT=Path(__file__).resolve().parents[3];V=ROOT/'research'/'voevodsky'/'results';R=ROOT/'research'/'benincasa'/'results';ps=(32003,32009,32027)
paths=(V/'cosmology_rank26_p_normal_K_q_canonical_signature_a14.json',V/'cosmology_rank26_p_normal_K_q_canonical_signature_p32009_a14.json',R/'cosmology_canonical_q_seed_signature_a14_p32027.json');pack=[json.loads(x.read_text()) for x in paths];M=math.prod(ps);B=math.isqrt(M//2)
...
out={'schema':'marici.benincasa.cosmology-canonical-seed-three-prime-reconstruction.v1','primes':list(ps),'modulus':M,'unique_height_bound':B,'typed_support_identical':True,'results':results,'all_18_reconstructed':all(x['rational'] for x in allterms),'all_replay_three_primes':all(x['replays_all'] for x in allterms),'characteristic_zero_relation_replayed':False,'passed':all(x['rational'] for x in allterms)};(R/'cosmology_canonical_seed_three_prime_reconstruction.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'bound':B,'counts':{k:v['reconstructed_count'] for k,v in results.items()},'rationals':[x['rational'] for x in allterms],'passed':out['passed']},indent=2))
