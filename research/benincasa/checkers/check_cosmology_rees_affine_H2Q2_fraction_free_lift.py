#!/usr/bin/env python3
"""Dependency-free modular/CRT/rational lift of an exact H^2Q^2 cokernel certificate."""
...
tgt=mul(target,mul(D,D));mons=sorted(set(tgt)|{m for c in cols for m in c},key=lambda x:(sum(x),x[0]));idx={m:i for i,m in enumerate(mons)};n=len(mons)
eq=[{idx[m]:v for m,v in c.items()} for c in cols];eq.append({**{idx[m]:v for m,v in tgt.items()},n:F(-1)})
state_path=HERE.parents[1]/'results'/'cosmology_rees_affine_H2Q2_crt_checkpoint_v2.json'
def isprime(q):
 if q<2:return False
 d=2
 while d*d<=q:
...
if state_path.exists():
 state=json.loads(state_path.read_text());res=[int(x) for x in state['residues']];M=int(state['modulus']);used=state['primes'];q=state['next_start']
...
for p in primes:
 ...
 M*=p
used+=primes
state_path.write_text(json.dumps({'modulus':str(M),'residues':[str(x) for x in res],'primes':used,'next_start':q})+'\n')
lam=[ratrec(x,M) for x in res]
if any(x is None for x in lam):
 print(json.dumps({'status':'checkpointed','prime_count':len(used),'crt_digits':len(str(M)),'unreconstructed':sum(x is None for x in lam)}));raise SystemExit(0)
assert all(sum(c.get(m,F(0))*lam[i] for i,m in enumerate(mons))==0 for c in cols)
pair=sum(tgt.get(m,F(0))*lam[i] for i,m in enumerate(mons));assert pair==1
cert=[{'monomial':[m[0],m[1]],'coefficient':str(lam[i])} for i,m in enumerate(mons) if lam[i]]
out={'schema':'marici.benincasa.cosmology-rees-affine-H2Q2-exact-certificate.v1','denominator':'H^2Q^2','numerator_caps':[19,15],'unknowns':len(cols),'equations':n,'primes':list(primes),'crt_modulus':str(M),'support_size':len(cert),'target_pairing':str(pair),'certificate':cert};path=HERE.parents[1]/'results'/'cosmology_rees_affine_H2Q2_exact_certificate.json';path.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({k:v for k,v in out.items() if k!='certificate'},indent=2))
