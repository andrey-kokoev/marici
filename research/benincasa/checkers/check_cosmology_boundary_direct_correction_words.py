...
def main():
 ...
 results={}
 for kp in (0,1):
  s=next(r for r in sp['records'] if r['k_pole']==kp and r['exponent']==[0,6]);t=tl[(kp,0,10)];row={}
  for term in s['terms']:
  ...
  results[f'k{kp}']={'source_coordinate':[0,6],'target_coordinate':[0,10],'normalized_trace_length':trace,'source_rows':len(coeff),'T_rows':sum(k[0]=='T' for k in coeff),'S_K_rows':sum(k[0]=='S_K' for k in coeff),'coefficient_sha256':digest.hexdigest(),'coefficients':[{'kind':k[0],'row_index':k[1],'coefficient':a} for k,a in sorted(coeff.items())],'reconstruction_verified':True}
 out={'schema':'marici.benincasa.cosmology-boundary-direct-correction-words.v1','status':'lower_edge_correction_words_reconstructed','from_ambient':12,'to_ambient':A,'results':results,'decision':'Representative lower-edge correction cells back-substitute completely to original T and S_K rows.','limitations':['two representatives per inclusion','row-index-dependent digest','single prime','uniform coefficient transport not yet tested'],'passed':True}
