...
RES=ROOT/'research'/'voevodsky'/'results';OUT=ROOT/'research'/'benincasa'/'results'/'cosmology_correction_word_naive_syzygy_audit.json'
def add(d,k,v):
...
def main():
 ...
 out={'schema':'marici.benincasa.cosmology-correction-word-naive-syzygy-audit.v1','status':'higher_coherence_syzygies_verified' if all(x['syzygy_verified'] for x in results.values()) else 'higher_coherence_syzygy_fails','map':'translate every source-row monomial exponent by (0,2), then subtract the next-inclusion correction word','results':results,'decision':'Correction source words are unequal representatives of the same transported source relation and differ by exact source syzygies.' if all(x['syzygy_verified'] for x in results.values()) else 'Correction-word differences are not zero source relations.','limitations':['lower-edge representatives only','single prime','one composable pair of inclusions','zero relation verified in finite ambient module'],'naive_syzygy_verified':all(x['syzygy_verified'] for x in results.values()),'passed':not any(x['syzygy_verified'] for x in results.values())}
