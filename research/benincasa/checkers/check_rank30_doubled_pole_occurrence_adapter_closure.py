#!/usr/bin/env python3
"""Test jet closure after adjoining the full source-forced doubled-pole occurrence packet."""
...
def strict_evaluate(raw,pres,free_set):
    ...
    if forbidden:
        ...
        raise RuntimeError("four-class doubled-pole occurrence adapter is not jet-closed")
...
jet.OUT=BEN/"results"/f"rank30-doubled-pole-adapter-source-{CHART.lower()}-p{P}.json"
try:
...
packet={
 "schema":"marici.rank30-doubled-pole-occurrence-adapter-closure.v1",
 "prime":P,"chart":CHART,
 ...
 "error":error,
 "checks":{
   "only_forced_occurrence_packet_adjoined":True,
   "no_ambient_truncation_performed":True,
   "candidate_has_expected_dimension":state["candidate_dimension"]==30,
 },
 "passed":state["candidate_dimension"]==30,
 "conclusion":(
   "The four-class doubled-pole occurrence adapter is closed under the tested source covariant jets."
   if outcome=="closed" else
   "The four-class doubled-pole occurrence adapter is not closed; the first forbidden residual identifies the next target grade."
 )
}
out=BEN/"results"/f"rank30-doubled-pole-adapter-{CHART.lower()}-p{P}.json"
