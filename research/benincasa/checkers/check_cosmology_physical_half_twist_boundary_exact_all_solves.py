...
VRES=ROOT/'research'/'voevodsky'/'results';A=rees.AMBIENT;OUT=ROOT/'research'/'benincasa'/'results'/f'cosmology_physical_half_twist_boundary_exact_all_a{A}.json';P=32003
def main():
 assert A in (12,14,16,18)
 protocol=json.loads((VRES/'cosmology_rank26_p_normal_protocol_gate.json').read_text());point=tuple(protocol['test_point_xyz']);nx=tuple(protocol['integral_unit_normals']['nx']);td=tuple(protocol['integral_unit_normals']['p_tangent_difference']);_,columns=rees.column_packet();packets={};orig=base.PRIME
 for p in exact.PS:
  base.PRIME=p;rees.charts.GAMMA=-pow(2,-1,p)%p;sp=list(rees.raw_relations(point,columns));T,_=adapter.derivative_rows(columns,point,td);dx,_=adapter.derivative_rows(columns,point,nx);nI=4*len(base.monomials_at_most(A));nK=64*len(base.monomials_at_most(A-4));packets[p]={'T':T,'S_K':sp[nI:nI+nK],'Q':sp[nI+nK:],'dx':dx}
 ...
 levels=(1,1,2,1,1);nI=4*len(base.monomials_at_most(A));records=[];cache={}
 for kp in (0,1):
  for e in [e for e in base.monomials_at_most(A-4) if sum(e)>=A-6]:
   ...
   assert recon==target;records.append({'k_pole':kp,'exponent':list(e),'dimension':len(erows),'equations':len(cols),'rank':rank,'full_reconstruction':True,'max_denominator':max(a.denominator for a in coef),'source_coefficients':[{'kind':k,'row_index':i,'numerator':a.numerator,'denominator':a.denominator} for (k,i),a in zip(origins,coef)]})
 base.PRIME=orig
 out={'schema':'marici.benincasa.cosmology-physical-half-twist-boundary-exact-all.v1','status':'physical_half_twist_all_boundary_targets_verified_over_Q','physical_gamma':'-1/2','ambient_relation_degree':A,'targets_verified':len(records),'dimension_max':max(r['dimension'] for r in records),'equations_max':max(r['equations'] for r in records),'max_denominator':max(r['max_denominator'] for r in records),'records':records,'decision':'Every top-three-degree K target has an exact rational full-source identity.','limitations':['finite ambient degree','p=32003-selected source closures','four-prime CRT rational reconstruction under the standard unique-height bound','does not yet prove direct/composite rational coherence'],'passed':True};OUT.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({k:v for k,v in out.items() if k!='records'},indent=2))
